    ...
    return ConditionSchema(
        ...
        protocols=[
            ProtocolEntry(
                ...
                protocol_name='TAVNS-EPI — Seizure Reduction via Auricular Vagus Nerve Stimulation',
                clinical_objective='Reduce seizure frequency in drug-resistant focal epilepsy via vagal afferent modulation',
                primary_targets=['Left cymba conchae (auricular branch CN X)'],
                secondary_targets=['NTS → LC → noradrenergic/GABAergic cortical modulation'],
                device_name='NEMOS (tVNS Technologies) or Parasym',
                session_structure='Electrode on left cymba conchae; 25 Hz, 250 µs pulse width; 0.5-3.0 mA below pain threshold; 4 hours/day continuous or 30 min x2/day',
                frequency='Daily for minimum 12 weeks',
                duration='4 hours/day continuous or 30 min x2/day',
                intensity_dose='0.5-3.0 mA (below pain threshold); 25 Hz; 250 µs',
                montage_roi='Left cymba conchae auricular electrode',
                laterality='Left ear',
                treatment_course='Daily for minimum 12 weeks; continue indefinitely if effective',
                monitoring='Seizure diary; heart rate; skin integrity; tolerability',
                expected_response='40% seizure reduction by 12 weeks (DeGiorgio 2013 RPED trial)',
                evidence_status='Moderate — DeGiorgio 2013 RPED trial (N=47): 40.5% vs 21.8% sham; Bauer 2016 confirmatory',
                cautions='Left ear only. Monitor heart rate. Does not replace AEDs. Adjunct therapy only. CE-marked in EU.',
                when_to_escalate='If <30% seizure reduction after 12 weeks, consider implanted VNS referral or add cathodal tDCS',
                when_to_stop_modify='Stop if bradycardia, severe ear pain, or seizure worsening',
                clinical_notes_extended='Non-invasive alternative to implanted VNS. Requires long treatment duration (12+ weeks) for full effect. Adjunct to AEDs.',
                key_citations=['DeGiorgio et al., 2013', 'Bauer et al., 2016'],
            ),
            ProtocolEntry(
                ...
                session_count=15,
                # Extended protocol fields
                protocol_name='TDCS-EPI-CAT — Cathodal tDCS Epileptogenic Focus Inhibition',
                clinical_objective='Reduce cortical excitability at epileptogenic focus via cathodal tDCS',
                primary_targets=['Epileptogenic focus (EEG/MRI-guided)'],
                secondary_targets=['Surrounding cortical excitability'],
                device_name='Newronika HDCkit (2-channel)',
                session_structure='Cathode (-): Over epileptogenic focus (EEG-guided, T3/T4 or F7/F8). Anode (+): Contralateral shoulder or Cz. 1.0-2.0 mA (start 1 mA), 20-30 min.',
                frequency='1 session/day, 5 days/week',
                duration='20-30 min per session',
                intensity_dose='1.0-2.0 mA (start 1 mA, titrate up)',
                montage_roi='Cathode (-): Over EZ (T3/T4 for TLE; F7/F8 for frontal; C3/C4 for motor). Anode (+): Contralateral shoulder or Cz',
                laterality='Ipsilateral to epileptogenic focus',
                treatment_course='10-15 sessions over 3 weeks',
                monitoring='Seizure diary; EEG monitoring recommended; impedance check; skin integrity',
                expected_response='Reduced seizure frequency after treatment block',
                evidence_status='Low — Fregni et al. 2006 pilot (N=6); multiple pilot studies; no large RCT',
                cautions='CRITICAL: Anodal tDCS is CONTRAINDICATED in epilepsy (increases excitability). Start at 1 mA. Doctor authorization mandatory. OFF-LABEL.',
                when_to_escalate='If <20% improvement after 15 sessions, consider taVNS or implanted VNS referral',
                when_to_stop_modify='Stop if seizure frequency increases, status epilepticus risk, or skin lesion',
                clinical_notes_extended='EEG localization required before treatment. Cathode must target confirmed epileptogenic zone. Anodal tDCS is absolutely contraindicated.',
            ...
            ProtocolEntry(
                ...
                session_count=10,
                # Extended protocol fields
                protocol_name='TMS-EPI — 1 Hz rTMS Epileptogenic Zone Inhibition',
                clinical_objective='Inhibit epileptogenic zone discharge via low-frequency repetitive TMS',
                primary_targets=['Epileptogenic focus (neuronavigation-guided)'],
                secondary_targets=['Surrounding cortical excitability'],
                device_name='MagVenture or Magstim TMS System',
                session_structure='1 Hz rTMS over epileptogenic zone (figure-8 coil); neuronavigation required; 900-1200 pulses per session',
                frequency='5 sessions/week',
                duration='15-20 min per session',
                intensity_dose='80-90% resting motor threshold; 1 Hz only',
                montage_roi='EEG/MRI-guided epileptogenic zone (figure-8 coil)',
                laterality='Ipsilateral to epileptogenic focus',
                treatment_course='10 sessions over 2 weeks',
                monitoring='Seizure diary; EEG monitoring; tolerability',
                expected_response='Reduced seizure frequency',
                evidence_status='Low — Tergau 1999; Cantello 2007; multiple small studies; inconsistent results',
                cautions='Low-frequency (1 Hz) ONLY. High-frequency rTMS is ABSOLUTE CONTRAINDICATION in epilepsy. Neuronavigation required. OFF-LABEL.',
                when_to_escalate='If no improvement, consider implanted VNS or epilepsy surgery referral',
                when_to_stop_modify='Stop if seizure worsening or status epilepticus risk',
                clinical_notes_extended='Low-frequency only — high-frequency rTMS is absolutely contraindicated in all epilepsy patients.',
                key_citations=['Tergau et al., 1999', 'Cantello et al., 2007'],
            ),
            ProtocolEntry(
                ...
                session_count=36,
                # Extended protocol fields
                protocol_name='TAVNS-EPI-DEP — Dual-Target Epilepsy + Depression Protocol',
                clinical_objective='Address both seizure frequency and interictal depression via combined taVNS and DLPFC tDCS',
                primary_targets=['Left cymba conchae (taVNS)', 'Left DLPFC (F3) — anodal tDCS'],
                secondary_targets=['NTS-LC pathway', 'Prefrontal-limbic circuits'],
                device_name='NEMOS/Parasym (taVNS) + Newronika HDCkit (tDCS)',
                session_structure='taVNS: 25 Hz, 250 µs, 4h/day. Then tDCS: F3 anodal, 1.5 mA, 20 min, 3x/week after taVNS session.',
                frequency='taVNS daily; tDCS 3x/week',
                duration='taVNS 4h/day; tDCS 20 min per session',
                intensity_dose='taVNS: 0.5-3.0 mA; tDCS: 1.5 mA',
                montage_roi='taVNS: left cymba conchae. tDCS: Anode (+): F3 (L-DLPFC). Cathode (-): right shoulder',
                laterality='Left (both modalities)',
                treatment_course='12 weeks taVNS + concurrent tDCS blocks',
                monitoring='Seizure diary; PHQ-9 monthly; heart rate; skin integrity',
                expected_response='Seizure reduction + mood improvement',
                evidence_status='Very low — no dedicated combined RCT; evidence extrapolated from separate taVNS/tDCS trials',
                cautions='OFF-LABEL combined protocol. SSRI interaction monitoring. Neurologist co-authorization required. Cathodal tDCS component only (no anodal over epileptogenic focus).',
                when_to_escalate='If depression persists, review antidepressant management; if seizures persist, VNS referral',
                when_to_stop_modify='Stop tDCS if seizure worsening; stop taVNS if bradycardia or ear pain',
                clinical_notes_extended='Dual-target protocol for EPI-DEP phenotype. Very limited combined evidence — requires clinical justification.',
                key_citations=['DeGiorgio et al., 2013'],
            ),
        ],
        ...
        eeg_reference_table=[
            {
                ...
                "role": 'Anodal tDCS for comorbid depression in EPI-DEP phenotype',
                "protocols_using": ['TAVNS-EPI-DEP'],
            },
            {
                "position": 'Fz',
                "brain_region": 'ACC / Midline Frontal',
                "role": 'Reference electrode; attention/cognition',
                "protocols_using": [],
            },
            {
                "position": 'C3',
                "brain_region": 'Left Motor Cortex',
                "role": 'Potential cathodal target for motor-area epileptogenic foci',
                "protocols_using": ['C-EPI-CAT'],
            },
            {
                "position": 'C4',
            ...
            {
                "position": 'T3',
                "brain_region": 'Left Temporal',
                "role": 'Cathodal target for left TLE epileptogenic focus',
                "protocols_using": ['C-EPI-CAT'],
            },
            {
                "position": 'T4',
                "brain_region": 'Right Temporal',
                "role": 'Cathodal target for right TLE epileptogenic focus',
                "protocols_using": ['C-EPI-CAT'],
            },
        ],
