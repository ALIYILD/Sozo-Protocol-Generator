    ...
    return ConditionSchema(
        ...
        protocols=[
            ProtocolEntry(
                ...
                protocol_name='TDCS-STROKE-M1 — Bilateral M1 Motor Recovery Protocol',
                clinical_objective='Restore interhemispheric excitability balance for upper limb motor recovery',
                primary_targets=['Ipsilesional M1 (C3 or C4) — anodal', 'Contralesional M1 — cathodal'],
                secondary_targets=['Corticospinal tract', 'Interhemispheric balance'],
                device_name='Newronika HDCkit (2-channel)',
                session_structure='Anode (+): Ipsilesional M1 (C3/C4). Cathode (-): Contralesional M1. 1.0-2.0 mA, 20-30 min. CONCURRENT with physiotherapy.',
                frequency='1 session/day, 5 days/week',
                duration='20-30 min per session',
                intensity_dose='1.0-2.0 mA',
                montage_roi='Anode (+): C3/C4 (ipsilesional M1). Cathode (-): C4/C3 (contralesional M1)',
                laterality='Ipsilesional anodal / Contralesional cathodal',
                treatment_course='10-20 sessions concurrent with physiotherapy',
                monitoring='Impedance check; FMA-UE; grip strength; Barthel Index; skin integrity',
                expected_response='FMA-UE >=6-point improvement; improved grip strength and ADL function',
                evidence_status='High — Cochrane review (Elsner et al. 2016, N=1272); IFCN Level A evidence; Hummel et al. 2005 RCT',
                cautions='OFF-LABEL. CRITICAL: stimulation MUST be concurrent with motor exercises. Identify lesioned hemisphere from neuroimaging.',
                when_to_escalate='If <6-point FMA-UE improvement after 10 sessions, switch montage (unilateral ipsilesional only); extend to 20 sessions',
                when_to_stop_modify='Stop if new neurological symptoms, skin lesion, or seizure',
                clinical_notes_extended='Best-evidenced tDCS application. Concurrent rehabilitation is mandatory — not sequential. Identify ipsilesional hemisphere from neuroimaging.',
                key_citations=['Elsner et al., 2016', 'Hummel et al., 2005', 'Lefaucheur et al., 2017'],
            ),
            ProtocolEntry(
                ...
                evidence_level=EvidenceLevel.HIGH, off_label=True, session_count=15,
                # Extended protocol fields
                protocol_name='TDCS-STROKE-APHASIA — Language Rehabilitation Protocol',
                clinical_objective='Enhance language network plasticity for aphasia recovery concurrent with speech therapy',
                primary_targets=["Left IFG / Broca's area (F7)"],
                secondary_targets=["Contralesional Broca's homologue (F8)"],
                device_name='Newronika HDCkit (2-channel)',
                session_structure="Anode (+): F7 (ipsilesional Broca's). Cathode (-): F8 or right shoulder. 1.5-2.0 mA, 20-30 min. CONCURRENT with speech-language therapy.",
                frequency='1 session/day, 5 days/week',
                duration='20-30 min per session',
                intensity_dose='1.5-2.0 mA',
                montage_roi='Anode (+): F7 (ipsilesional IFG / Broca). Cathode (-): F8 (contralesional) or right shoulder',
                laterality='Left (ipsilesional) for left-hemisphere stroke',
                treatment_course='10-15 sessions concurrent with speech-language therapy',
                monitoring='Impedance check; naming accuracy; verbal fluency; BNT; skin integrity',
                expected_response='Improved naming, verbal fluency, and connected speech',
                evidence_status='High — multiple RCTs demonstrate superior naming/fluency outcomes; IFCN evidence',
                cautions='OFF-LABEL. Concurrent speech therapy MANDATORY. Naming/repetition/picture description tasks during stimulation.',
                when_to_escalate='If <20% improvement after 10 sessions, consider contralesional cathodal-only montage',
                when_to_stop_modify='Stop if seizure, new neurological symptoms, or skin lesion',
                clinical_notes_extended="Ipsilesional anodal preferred when residual Broca's cortex survives. Concurrent SLT is mandatory.",
                key_citations=['Fregni et al., 2005', 'Elsner et al., 2016'],
            ),
            ProtocolEntry(
                ...
                # Extended protocol fields
                protocol_name='TDCS-STROKE-COG — Post-Stroke Cognitive Rehabilitation',
                clinical_objective='Improve executive function and attention in post-stroke cognitive impairment',
                primary_targets=['Bilateral DLPFC (F3/F4) or unilateral ipsilesional'],
                secondary_targets=['Frontoparietal attention network'],
                device_name='Newronika HDCkit (2-channel)',
                session_structure='Anode (+): F3/F4 (bilateral DLPFC) or ipsilesional. Cathode (-): right shoulder or Oz. 2 mA, 20 min. Concurrent cognitive training.',
                frequency='1 session/day, 5 days/week',
                duration='20 min per session',
                intensity_dose='2 mA',
                montage_roi='Anode (+): F3/F4 (bilateral DLPFC). Cathode (-): right shoulder or Oz',
                laterality='Bilateral or ipsilesional',
                treatment_course='10-15 sessions concurrent with cognitive training',
                monitoring='Impedance check; MoCA; attention/working memory tests; skin integrity',
                expected_response='Improved attention, processing speed, and working memory',
                evidence_status='Moderate — DLPFC tDCS for PSCI supported by emerging evidence',
                cautions='OFF-LABEL. Concurrent cognitive training essential. Rule out post-stroke depression.',
                when_to_escalate='If <20% improvement after 10 sessions, screen for PSD and address mood first',
                when_to_stop_modify='Stop if confusion, new neurological symptoms, or skin lesion',
                clinical_notes_extended='Concurrent cognitive training (attention, working memory tasks) essential for activity-dependent plasticity.',
                key_citations=['Lefaucheur et al., 2017'],
            ),
            ProtocolEntry(
                ...
                # Extended protocol fields
                protocol_name='TDCS-STROKE-PSD — Post-Stroke Depression Protocol',
                clinical_objective='Treat post-stroke depression via left DLPFC anodal stimulation',
                primary_targets=['Left DLPFC (F3)'],
                secondary_targets=['CEN-DMN-limbic circuits'],
                device_name='Newronika HDCkit (2-channel)',
                session_structure='Anode (+): F3 (L-DLPFC). Cathode (-): Fp2 (R supraorbital). 2 mA, 30 min.',
                frequency='1 session/day, 5 days/week',
                duration='30 min per session',
                intensity_dose='2 mA',
                montage_roi='Anode (+): F3 (L-DLPFC). Cathode (-): Fp2 (R supraorbital)',
                laterality='Left DLPFC anodal',
                treatment_course='10-15 sessions over 3 weeks',
                monitoring='Impedance check; PHQ-9 biweekly; C-SSRS EVERY session; skin integrity',
                expected_response='PHQ-9 >=50% reduction by end of block',
                evidence_status='Moderate — extrapolated from robust MDD tDCS literature; limited PSD-specific RCTs',
                cautions='OFF-LABEL. C-SSRS mandatory at every session. Undertreated PSD severely limits stroke rehabilitation.',
                when_to_escalate='If <20% improvement, add CES; review SSRI/SNRI regimen',
                when_to_stop_modify='Stop if suicidal ideation increase, manic switch, or skin lesion',
                clinical_notes_extended='MDD montage adapted for PSD. Undertreated depression limits all rehabilitation engagement.',
                key_citations=['Brunoni et al., 2013'],
            ),
        ],
        ...
        eeg_reference_table=[
            {
                ...
                "role": 'Anodal — ipsilesional M1 (right hemiparesis)',
                "protocols_using": ['C-STROKE-MOTOR'],
            },
            {
                "position": 'C4',
                "brain_region": 'Right Primary Motor Cortex',
                "role": 'Cathodal — contralesional M1 inhibition or ipsilesional M1 (left hemiparesis)',
                "protocols_using": ['C-STROKE-MOTOR'],
            },
            {
                "position": 'F3',
                "brain_region": 'Left DLPFC',
                "role": 'Cognitive/language/depression rehabilitation target',
                "protocols_using": ['C-COG-STROKE', 'C-PSD'],
            },
            {
                "position": 'F4',
                "brain_region": 'Right DLPFC',
                "role": 'Post-stroke cognitive or depression target',
                "protocols_using": ['C-COG-STROKE'],
            },
            {
                "position": 'F7',
                "brain_region": "Left IFG / Broca's area proxy",
                "role": 'Anodal — aphasia rehabilitation',
                "protocols_using": ['C-APHASIA'],
            },
        ],
