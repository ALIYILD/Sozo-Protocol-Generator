    ...
    return ConditionSchema(
        ...
        protocols=[
            ProtocolEntry(
                ...
                protocol_name='TMS-DYT-INHIB — Inhibitory Sensorimotor Cortex TMS',
                clinical_objective='Reduce maladaptive cortical hyperexcitability and normalize motor maps in focal dystonia',
                primary_targets=['Contralateral Primary Motor Cortex (C3/C4)'],
                secondary_targets=['Premotor cortex'],
                device_name='MagVenture or Magstim TMS System',
                session_structure='1 Hz rTMS or cTBS over contralateral M1 (neuronavigation-guided); concurrent sensorimotor retraining post-stimulation',
                frequency='5 sessions/week',
                duration='15-20 min per session (900-1200 pulses at 1 Hz; or 600 pulses cTBS)',
                intensity_dose='90% resting motor threshold',
                montage_roi='C3 or C4 (contralateral M1) via neuronavigation',
                laterality='Contralateral to affected limb',
                treatment_course='10-15 sessions over 3 weeks',
                monitoring='TWSTRS or BFM-DRS at baseline and 4 weeks; video assessment; skin integrity',
                expected_response='Short-term improvement in dystonic posturing and task-specific triggering',
                evidence_status='Moderate — multiple small studies demonstrate short-term improvement in focal dystonia; no large RCT',
                cautions='OFF-LABEL. CONTRAINDICATED if DBS in situ. Low-frequency (1 Hz) only — high-frequency rTMS absolutely contraindicated in dystonia. Coordinate with botulinum toxin schedule.',
                when_to_escalate='If no improvement after 10 sessions, reassess botulinum toxin regimen; consider GPi-DBS referral',
                when_to_stop_modify='Stop if worsening dystonia, seizure, or any DBS interaction',
                clinical_notes_extended='Combine with sensorimotor retraining for additive plasticity reversal. Synergistic with botulinum toxin — do not discontinue injections.',
                key_citations=['Breakefield et al., 2008', 'Albanese et al., 2013'],
            ),
            ProtocolEntry(
                ...
                # Extended protocol fields
                protocol_name='TDCS-DYT-SMC — Sensorimotor Cortex Cathodal tDCS',
                clinical_objective='Reduce motor cortex excitability and normalize dystonic cortical plasticity',
                primary_targets=['Contralateral Primary Sensorimotor Cortex (C3/C4)'],
                secondary_targets=['Premotor areas'],
                device_name='Newronika HDCkit (2-channel)',
                session_structure='Cathode (-): C3 or C4 (contralateral sensorimotor). Anode (+): Contralateral shoulder or Fp2. 1.5-2.0 mA, 20 min.',
                frequency='1 session/day, 5 days/week',
                duration='20 min per session',
                intensity_dose='1.5-2.0 mA',
                montage_roi='Cathode (-): C3 or C4 (contralateral sensorimotor cortex). Anode (+): Contralateral shoulder or Fp2',
                laterality='Contralateral to affected limb',
                treatment_course='10-15 sessions over 3 weeks',
                monitoring='Impedance check; TWSTRS or BFM-DRS biweekly; motor retraining progress; skin integrity',
                expected_response='Reduced dystonic posturing and improved voluntary motor control',
                evidence_status='Low — pilot data for focal hand dystonia; limited controlled studies',
                cautions='OFF-LABEL. Adjunct to botulinum toxin — do not replace. DBS in situ: verify absence before treatment.',
                when_to_escalate='If <20% improvement after 10 sessions, reassess botulinum toxin; consider TMS or GPi-DBS referral',
                when_to_stop_modify='Stop if worsening dystonia, burning, skin lesion, or neurological change',
                clinical_notes_extended='Motor retraining tasks during stimulation — concurrent rehabilitation paradigm enhances plasticity reversal.',
                key_citations=['Breakefield et al., 2008'],
            ),
            ProtocolEntry(
                ...
                # Extended protocol fields
                protocol_name='TDCS-DYT-BILAT — Bilateral Sensorimotor tDCS for Segmental/Generalized',
                clinical_objective='Reduce bilateral motor cortex hyperexcitability in segmental/generalized dystonia',
                primary_targets=['Bilateral Sensorimotor Cortex (C3, C4)'],
                secondary_targets=['SMA (FCz)'],
                device_name='Newronika HDCkit (2-channel)',
                session_structure='Ch1 Cathode (-): C3. Ch2 Cathode (-): C4 (bilateral cathodal — requires dual-channel). Anode (+): Bilateral shoulders. 1.5 mA bilateral, 20 min.',
                frequency='1 session/day, 5 days/week',
                duration='20 min per session',
                intensity_dose='1.5 mA bilateral',
                montage_roi='Cathode (-): C3 (L-M1) and C4 (R-M1). Anode (+): Bilateral shoulders',
                laterality='Bilateral',
                treatment_course='15-20 sessions over 4-5 weeks',
                monitoring='Impedance check; BFM-DRS at baseline and 4 weeks; video assessment; skin integrity',
                expected_response='Modest reduction in dystonic severity; primarily bridge/adjunct to DBS or medication',
                evidence_status='Low — investigational; no controlled data for bilateral tDCS in generalized dystonia',
                cautions='OFF-LABEL. GPi-DBS referral should be discussed with every generalized dystonia patient. tDCS is investigational adjunct only.',
                when_to_escalate='GPi-DBS referral is primary escalation for generalized dystonia. Neurologist review mandatory.',
                when_to_stop_modify='Stop if worsening dystonia, burning, skin lesion, or neurological change',
                clinical_notes_extended='GPi-DBS referral required for generalized dystonia. tDCS is investigational bridge/adjunct only.',
                key_citations=['Vidailhet et al., 2005', 'Albanese et al., 2013'],
            ),
        ],
        ...
        eeg_reference_table=[
            {
                "position": 'C3',
                "brain_region": 'Left Primary Motor Cortex (M1)',
                "role": 'Cathodal inhibitory — reduce maladaptive hyperplasticity (right-hand dystonia)',
                "protocols_using": ['C-DYT-SMC-CATHODAL', 'C-DYT-BILATERAL-SMC'],
            },
            {
                "position": 'C4',
                "brain_region": 'Right Primary Motor Cortex (M1)',
                "role": 'Cathodal inhibitory — reduce hyperplasticity (left-hand dystonia)',
                "protocols_using": ['C-DYT-SMC-CATHODAL', 'C-DYT-BILATERAL-SMC'],
            },
            {
                "position": 'FCz',
                "brain_region": 'Supplementary Motor Area (SMA)',
                "role": 'SMA modulation — premotor hyperactivation',
                "protocols_using": ['C-DYT-BILATERAL-SMC'],
            },
            {
                "position": 'F3',
                "brain_region": 'Left DLPFC',
                "role": 'Executive/cognitive support',
                "protocols_using": [],
            },
        ],
