...
def build_tinnitus_condition() -> ConditionSchema:
    ...
    return ConditionSchema(
        ...
        eeg_reference_table=[
            {"position": "T3", "brain_region": "Left Temporal / Auditory Cortex", "role": "PRIMARY cathodal target — auditory cortex hyperexcitability reduction; Heschl's gyrus approximation", "protocols_using": ["C-TIN-BIFRONTAL", "C-TIN-AC"]},
            {"position": "T4", "brain_region": "Right Temporal / Auditory Cortex", "role": "Bilateral temporal access for bilateral tinnitus; contralateral auditory cortex", "protocols_using": []},
            {"position": "C3", "brain_region": "Left Primary Motor Cortex (M1)", "role": "M1/somatosensory cortex — somatic tinnitus modulation; cervicogenic component", "protocols_using": []},
            {"position": "C4", "brain_region": "Right Primary Motor Cortex (M1)", "role": "Bilateral M1/somatosensory for somatic tinnitus phenotype", "protocols_using": []},
            {"position": "F3", "brain_region": "Left DLPFC", "role": "Top-down attentional suppression of tinnitus; distress and anxiety modulation", "protocols_using": ["C-TIN-BIFRONTAL"]},
        ],
        ...
        protocols=[
            ProtocolEntry(
                ...
                protocol_name="C-TIN-BIFRONTAL — Bifrontal-Temporal Dual Target Tinnitus Protocol",
                clinical_objective="Dual-target: inhibit auditory cortex hyperexcitability (T3) + upregulate top-down attention suppression (F3)",
                primary_targets=["T3 (Left temporal — cathodal / auditory cortex inhibition)"],
                secondary_targets=["F3 (Left DLPFC — anodal / attention regulation) or Fp1"],
                device_name="Newronika HDCkit (2-channel)",
                session_structure="Cathode T3 (auditory cortex); Anode F3 or Fp1. 1.5–2.0 mA, 20–30 min. Vanneste & De Ridder montage.",
                frequency="5 sessions/week",
                duration="20–30 min per session",
                intensity_dose="1.5–2.0 mA",
                montage_roi="Cathode (−): T3 (L-temporal). Anode (+): F3 (L-DLPFC) or Fp1",
                laterality="Left temporal dominant (bilateral for bilateral tinnitus)",
                treatment_course="10–15 sessions over 3 weeks",
                monitoring="VAS tinnitus loudness and distress at every session; THI at Week 4 and endpoint; skin integrity",
                expected_response="Reduced tinnitus loudness and distress; improved attention control over tinnitus",
                evidence_status="Medium — Vanneste & De Ridder 2011; Shekhawat et al. 2013 systematic review",
                cautions="T3 electrode placement precision critical — skull anatomy variation affects targeting",
                when_to_escalate="If no improvement at Week 4, switch to unilateral temporal or add CES for anxiety component",
                when_to_stop_modify="Standard tDCS stop criteria; reduce intensity if tinnitus exacerbation persists beyond 3 sessions",
                clinical_notes_extended="Dual-target montage addresses both auditory source and attention/distress amplifiers. Monitor VAS at start and end of every session.",
                key_citations=["Vanneste & De Ridder, 2011", "Shekhawat et al., 2013"],
            ),
            ProtocolEntry(
                ...
                evidence_level=EvidenceLevel.MEDIUM, off_label=True, session_count=15,
                # -- SOZO Protocol Handbook fields --
                protocol_name="C-TIN-AC — Auditory Cortex Cathodal Tinnitus Suppression Protocol",
                clinical_objective="Reduce auditory cortex hyperexcitability underlying tinnitus phantom sound generation",
                primary_targets=["T3 (Left temporal cortex — cathodal)"],
                secondary_targets=["Right shoulder or Fp2 (anodal reference)"],
                device_name="Newronika HDCkit (2-channel)",
                session_structure="Cathode T3 (left temporal); Anode right shoulder or Fp2. 1.5–2.0 mA, 20 min.",
                frequency="5 sessions/week",
                duration="20 min per session",
                intensity_dose="1.5–2.0 mA",
                montage_roi="Cathode (−): T3 (L-temporal). Anode (+): right shoulder or Fp2",
                laterality="Left temporal",
                treatment_course="10–15 sessions over 3 weeks",
                monitoring="VAS tinnitus loudness and distress at every session; THI at endpoint",
                expected_response="Reduced tinnitus loudness via auditory cortex inhibition",
                evidence_status="Medium — Vanneste & De Ridder 2011; Lehtimaki et al. 2013; inconsistent results across studies",
                cautions="Electrode placement precision critical; inconsistent findings across studies",
                when_to_escalate="If no improvement, switch to bifrontal-temporal dual-target protocol",
                when_to_stop_modify="Standard tDCS stop criteria; modify electrode position if no response",
                clinical_notes_extended="Results inconsistent across studies — electrode placement precision is the key challenge. T3 approximates Heschl's gyrus but anatomy varies.",
                key_citations=["Vanneste & De Ridder, 2011", "Lehtimaki et al., 2013"],
            ),
            ProtocolEntry(
                ...
                # -- SOZO Protocol Handbook fields --
                protocol_name="TAVNS-TIN — taVNS Auditory Modulation & Anxiety Adjunct for Tinnitus",
                clinical_objective="Modulate auditory cortex excitability via vagal-NTS-auditory pathways; address tinnitus-related anxiety",
                primary_targets=["Auricular branch of vagus nerve → NTS → auditory cortex modulation"],
                secondary_targets=["Limbic network (tinnitus-related anxiety and insomnia)"],
                device_name="NEMOS (tVNS Technologies) or Parasym",
                session_structure="Electrode placement on left cymba conchae; 25 Hz, 250 µs; below pain threshold; 30 min",
                frequency="Daily adjunct during tDCS block",
                duration="30 min per session",
                intensity_dose="Below pain threshold",
                montage_roi="Left cymba conchae (auricular vagus branch)",
                laterality="Left auricular",
                treatment_course="Daily for duration of tDCS block (3–4 weeks)",
                monitoring="VAS tinnitus; GAD-7 anxiety; heart rate; auricular skin integrity",
                expected_response="Reduced tinnitus perception via auditory pathway modulation; reduced tinnitus-related anxiety",
                evidence_status="Low — Lehtimaki et al. 2013 early evidence; investigational for tinnitus",
                cautions="Investigational; adjunct to tDCS; informed consent required",
                when_to_escalate="If no tinnitus improvement, focus on primary tDCS protocol optimization",
                when_to_stop_modify="Stop if vagal side effects or auricular skin irritation",
                clinical_notes_extended="taVNS modulates auditory cortex via NTS-auditory pathway connections. Also addresses tinnitus-related anxiety and insomnia.",
                key_citations=["Lehtimaki et al., 2013"],
            ),
            ProtocolEntry(
                ...
                # -- SOZO Protocol Handbook fields --
                protocol_name="CES-TIN — Alpha-Stim CES Sleep & Anxiety Adjunct for Tinnitus",
                clinical_objective="Address tinnitus-related anxiety and insomnia that amplify tinnitus distress cycle",
                primary_targets=["Limbic and autonomic networks via transcranial microcurrent"],
                secondary_targets=["SN downregulation (indirect)", "Thalamocortical sleep entrainment"],
                device_name="Alpha-Stim® CS",
                session_structure="Bilateral earlobe clip electrodes; 0.5 Hz; 100–200 µA; 40–60 min before sleep",
                ...
                treatment_course="Nightly for duration of tDCS block; maintenance 3×/week",
                monitoring="GAD-7 anxiety; PSQI sleep; VAS tinnitus distress; skin integrity",
                expected_response="Reduced tinnitus-related anxiety and insomnia; improved distress coping",
                evidence_status="Medium — FDA-cleared for anxiety and insomnia; addresses tinnitus comorbidities",
                cautions="For tinnitus-anxiety phenotype; anxiety amplifies tinnitus distress disproportionately",
                when_to_escalate="If anxiety not improving, add taVNS or pharmacological anxiolytic",
                when_to_stop_modify="Stop if skin irritation at earlobe sites",
                clinical_notes_extended="Address anxiety first with CES — anxiety amplifies tinnitus distress disproportionately. Sleep restoration reduces tinnitus-related distress cycle.",
        ...
        platoscience_variants=[
            PlatoScienceVariant(
                variant_id="C-TIN-BIFRONTAL-PS", protocol_id="C-TIN-BIFRONTAL",
                label="PlatoScience Bifrontal-Temporal — Dual Target Tinnitus",
                parameters={"program": "Focus", "electrode_config": "F3 (left DLPFC) or Fp1", "intensity": "1.5-2.0 mA", "duration": "20-30 min", "ramp": "30 sec"},
                notes="Maps to C-TIN-BIFRONTAL protocol. PlatoScience Focus program.",
            ),
            PlatoScienceVariant(
                variant_id="C-TIN-AC-PS", protocol_id="C-TIN-AC",
                label="PlatoScience Auditory Cortex Cathodal — Tinnitus Suppression",
                parameters={"program": "Focus", "electrode_config": "Right shoulder or Fp2", "intensity": "1.5-2.0 mA", "duration": "20 min", "ramp": "30 sec"},
                notes="Maps to C-TIN-AC protocol. PlatoScience Focus program.",
            ),
        ],
