    ...
    return ConditionSchema(
        ...
        protocols=[
            ProtocolEntry(
                ...
                protocol_name='TDCS-TBI-COG — Cognitive Rehabilitation DLPFC Protocol',
                clinical_objective='Improve attention, working memory, and processing speed post-TBI',
                primary_targets=['Left DLPFC (F3) or bilateral F3+F4'],
                secondary_targets=['CEN executive circuits'],
                device_name='Newronika HDCkit (2-channel)',
                session_structure='Anode (+): F3 (L-DLPFC) or bilateral F3+F4. Cathode (-): Fp2 or right shoulder. 1.5-2.0 mA (start 1.5), 20 min. Concurrent cognitive training.',
                frequency='1 session/day, 5 days/week',
                duration='20 min per session',
                intensity_dose='1.5-2.0 mA (start 1.5 mA for first 2-3 sessions)',
                montage_roi='Anode (+): F3 (L-DLPFC). Cathode (-): Fp2 or right shoulder',
                laterality='Left anodal or bilateral',
                treatment_course='10-15 sessions over 3 weeks',
                monitoring='Impedance check; MoCA; PCSS; cognitive battery; skin integrity',
                expected_response='Improved attention and processing speed by Week 2-3; PCSS reduction >=30%',
                evidence_status='Moderate — Pape et al. 2015 pilot; McIntire et al. 2017; multiple small studies',
                cautions='OFF-LABEL. Start at 1.5 mA due to TBI-related altered cortical excitability. Concurrent cognitive training recommended. Monitor headache.',
                when_to_escalate='If <20% improvement after 10 sessions, add CES for sleep/anxiety; screen for untreated sleep disorder',
                when_to_stop_modify='Stop if headache worsening, skin lesion, or seizure (elevated risk in moderate-severe TBI)',
                clinical_notes_extended='Start conservatively at 1.5 mA in TBI due to altered cortical excitability. Concurrent cognitive training is recommended.',
                key_citations=['Pape et al., 2015', 'McIntire et al., 2017'],
            ),
            ProtocolEntry(
                ...
                # Extended protocol fields
                protocol_name='TAVNS-TBI — Autonomic & Fatigue Adjunct Protocol',
                clinical_objective='Address autonomic dysfunction and fatigue in TBI via vagal modulation',
                primary_targets=['Left cymba conchae (auricular CN X)'],
                secondary_targets=['Autonomic rebalancing', 'BDNF neuroplasticity upregulation'],
                device_name='NEMOS (tVNS Technologies) or Parasym',
                session_structure='Electrode on left cymba conchae; 25 Hz, 200-250 µs; below pain threshold; 30 min',
                frequency='Daily adjunct during tDCS block',
                duration='30 min per session',
                intensity_dose='Below pain threshold (0.5-4.0 mA); 25 Hz',
                montage_roi='Left cymba conchae auricular electrode',
                laterality='Left ear',
                treatment_course='Daily during tDCS treatment block',
                monitoring='Heart rate; HRV; fatigue scales; skin integrity; tolerability',
                expected_response='Reduced fatigue and improved autonomic balance',
                evidence_status='Low — investigational in TBI; rationale from autonomic dysfunction literature',
                cautions='Investigational in TBI. Monitor heart rate — autonomic sensitivity in TBI. Left ear only.',
                when_to_escalate='If no improvement after 4 weeks, reassess fatigue etiology',
                when_to_stop_modify='Stop if bradycardia, ear pain, or autonomic instability',
                clinical_notes_extended='Autonomic dysfunction (reduced HRV) is prevalent in TBI. Vagal-mediated BDNF upregulation may enhance neuroplasticity.',
            ...
            ProtocolEntry(
                ...
                # Extended protocol fields
                protocol_name='CES-TBI — Sleep, Anxiety & Mood Adjunct',
                clinical_objective='Address sleep, anxiety, and mood triad in TBI/PCS',
                ...
                laterality='Bilateral',
                treatment_course='Daily during treatment block; then as-needed',
                monitoring='PSQI; PHQ-9; GAD-7; skin integrity',
                expected_response='Improved sleep and reduced anxiety within 1-2 weeks',
                evidence_status='Moderate — CES FDA-cleared for anxiety, depression, insomnia',
                cautions='Adjunctive only. Non-pharmacological option reduces medication burden in TBI.',
                when_to_escalate='If symptoms persist, review sleep hygiene and consider sleep study',
                when_to_stop_modify='Stop if skin irritation',
                clinical_notes_extended='Targets the prevalent sleep-anxiety-mood triad in TBI/PCS. Non-pharmacological adjunct.',
            ...
            ProtocolEntry(
                ...
                # Extended protocol fields
                protocol_name='TDCS-TBI-MOOD — Post-TBI Depression Protocol',
                clinical_objective='Treat post-TBI depression via left DLPFC anodal stimulation',
                ...
                evidence_status='Moderate — MDD tDCS protocol applied to post-TBI depression',
                cautions='OFF-LABEL. C-SSRS mandatory. PHQ-9 at each visit. Monitor for headache exacerbation.',
                when_to_escalate='If <20% improvement, add CES; review antidepressant management',
                when_to_stop_modify='Stop if suicidal ideation increase or skin lesion',
                clinical_notes_extended='Post-TBI depression is treated with MDD protocol rationale. C-SSRS monitoring mandatory.',
        ...
        platoscience_variants=[
            PlatoScienceVariant(
                variant_id="C-TBI-COG-PS", protocol_id="C-TBI-COG",
                label="PlatoScience Cognitive Rehabilitation — DLPFC tDCS",
                parameters={"program": "Think", "electrode_config": "F3 (left DLPFC) or bilateral F3+F4", "intensity": "1.5-2.0 mA (start 1.5 mA for first 2-3 sessions in TBI)", "duration": "20 min", "ramp": "30 sec"},
                notes="Maps to C-TBI-COG protocol. PlatoScience Think program.",
            ),
            PlatoScienceVariant(
                variant_id="C-TBI-MOOD-PS", protocol_id="C-TBI-MOOD",
                label="PlatoScience Post-TBI Depression — DLPFC tDCS",
                parameters={"program": "Think", "electrode_config": "F3 (left DLPFC)", "intensity": "2.0 mA", "duration": "30 min", "ramp": "30 sec"},
                notes="Maps to C-TBI-MOOD protocol. PlatoScience Think program.",
            ),
        ],
        ...
        eeg_reference_table=[
            {
                ...
                "role": 'Anodal — cognitive rehabilitation and mood',
                "protocols_using": ['C-TBI-COG', 'C-TBI-MOOD'],
            },
            {
                "position": 'F4',
                "brain_region": 'Right DLPFC',
                "role": 'Bilateral cognitive target',
                "protocols_using": ['C-TBI-COG'],
            },
            {
                "position": 'Fz',
                "brain_region": 'ACC / Midline Frontal',
                "role": 'Attention/executive support',
                "protocols_using": [],
            },
            {
                "position": 'C3',
                "brain_region": 'Left Motor Cortex',
                "role": 'Motor rehabilitation (injury-dependent)',
                "protocols_using": [],
            },
            {
                "position": 'C4',
