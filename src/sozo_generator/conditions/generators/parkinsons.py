    ...
    return ConditionSchema(
        ...
        protocols=[
            ProtocolEntry(
                ...
                protocol_name='TDCS-PD-M1 — Motor Bradykinesia & Rigidity Protocol',
                clinical_objective='Improve bradykinesia and rigidity via bilateral M1 anodal tDCS',
                primary_targets=['Bilateral M1 (C3/C4)'],
                secondary_targets=['Corticospinal motor output'],
                device_name='Newronika HDCkit (2-channel)',
                session_structure='Ch1 Anode (+): C3 (L-M1). Ch2 Anode (+): C4 (R-M1). Cathode (-): Fp1/Fp2 bilateral supraorbital. 2 mA, 20 min.',
                frequency='1 session/day, 5 days/week',
                duration='20 min per session',
                intensity_dose='2 mA per channel',
                montage_roi='Anode (+): C3 (L-M1), C4 (R-M1). Cathode (-): Fp1/Fp2 (bilateral supraorbital)',
                laterality='Bilateral',
                treatment_course='10-15 sessions over 3 weeks',
                monitoring='Impedance check; UPDRS-III biweekly; gait speed; skin integrity',
                expected_response='Improved motor speed and reduced rigidity; UPDRS-III improvement >=20%',
                evidence_status='High — multiple RCTs demonstrate M1 anodal tDCS improvements in PD motor function',
                cautions='OFF-LABEL. Concurrent physiotherapy recommended. Coordinate with levodopa timing (ON-state preferred).',
                when_to_escalate='If <20% UPDRS-III improvement after 10 sessions, reassess targeting; consider TPS M1 protocol',
                when_to_stop_modify='Stop if skin lesion, dyskinesia worsening, or neurological change',
                clinical_notes_extended='Best-evidenced tDCS application in PD. Deliver during ON-state for optimal motor cortex engagement.',
                key_citations=['Fregni et al., 2006', 'Benninger et al., 2010'],
            ),
            ProtocolEntry(
                ...
                # Extended protocol fields
                protocol_name='TDCS-PD-GAIT — Gait & Dual-Task Protocol',
                clinical_objective='Improve gait speed and dual-task performance via SMA/M1 combined stimulation',
                primary_targets=['SMA (Fz)', 'M1 (C3/C4)'],
                secondary_targets=['Frontal gait circuits'],
                device_name='Newronika HDCkit (2-channel)',
                session_structure='Anode (+): Fz (SMA) + C3/C4 (M1). Cathode (-): Fp1/Fp2. 2 mA, 20 min.',
                frequency='1 session/day, 5 days/week',
                duration='20 min per session',
                intensity_dose='2 mA',
                montage_roi='Anode (+): Fz (SMA), C3/C4 (M1). Cathode (-): Fp1/Fp2',
                laterality='Midline + bilateral',
                treatment_course='10-15 sessions over 3 weeks',
                monitoring='Impedance check; TUG test; 10m walk test; dual-task walking; FOG-Q; skin integrity',
                expected_response='Improved gait speed, reduced FOG episodes, better dual-task performance',
                evidence_status='Moderate — SMA+M1 stimulation supported for PIGD phenotype',
                cautions='OFF-LABEL. Concurrent gait physiotherapy during stimulation recommended. Fall risk assessment.',
                when_to_escalate='If <20% improvement, consider TPS SMA protocol',
                when_to_stop_modify='Stop if gait worsening, falls increase, or skin lesion',
                clinical_notes_extended='SMA hypoactivity contributes to FOG and gait initiation failure. Combined SMA+M1 has additive benefits.',
                key_citations=['Kaski et al., 2014'],
            ),
            ProtocolEntry(
                ...
                evidence_level=EvidenceLevel.MEDIUM, off_label=True, session_count=15,
                # Extended protocol fields
                protocol_name='TDCS-PD-COG — Cognition & Executive Function Protocol',
                clinical_objective='Improve executive function and working memory in PD-MCI',
                primary_targets=['Bilateral DLPFC (F3/F4)'],
                secondary_targets=['Frontostriatal executive circuits'],
                device_name='Newronika HDCkit (2-channel)',
                session_structure='Ch1 Anode (+): F3 (L-DLPFC). Ch2 Anode (+): F4 (R-DLPFC). Cathode (-): Fp1/Fp2 or shoulder. 2 mA, 20 min.',
                frequency='1 session/day, 5 days/week',
                duration='20 min per session',
                intensity_dose='2 mA per channel',
                montage_roi='Anode (+): F3 (L-DLPFC), F4 (R-DLPFC). Cathode (-): Fp1/Fp2 or shoulder',
                laterality='Bilateral prefrontal',
                treatment_course='10-15 sessions over 3 weeks',
                monitoring='Impedance check; MoCA; executive subtests; skin integrity',
                expected_response='Improved working memory and cognitive flexibility',
                evidence_status='Moderate — sham-controlled trials show cognitive improvement with DLPFC tDCS in PD',
                cautions='OFF-LABEL. Screen for PD-dementia (DLB features). Concurrent cognitive training recommended.',
                when_to_escalate='If <20% improvement, consider TPS DLPFC protocol',
                when_to_stop_modify='Stop if confusion, hallucinations, or skin lesion',
                clinical_notes_extended='Targets frontostriatal executive pathways. Concurrent cognitive training enhances outcomes.',
                key_citations=['Doruk et al., 2014'],
            ),
            ProtocolEntry(
                ...
                evidence_level=EvidenceLevel.MEDIUM, off_label=True, session_count=15,
                # Extended protocol fields
                protocol_name='TDCS-PD-DEP — Depression & Mood Protocol',
                clinical_objective='Treat PD-related depression and apathy via left DLPFC anodal stimulation',
                primary_targets=['Left DLPFC (F3)'],
                secondary_targets=['Limbic-prefrontal circuits'],
                device_name='Newronika HDCkit (2-channel)',
                session_structure='Anode (+): F3 (L-DLPFC). Cathode (-): F8 (right supraorbital or R-DLPFC). 2 mA, 30 min.',
                frequency='1 session/day, 5 days/week',
                duration='30 min per session',
                intensity_dose='2 mA',
                montage_roi='Anode (+): F3 (L-DLPFC). Cathode (-): F8 (R supraorbital or R-DLPFC)',
                laterality='Left hemisphere bias',
                treatment_course='10-15 sessions over 3 weeks',
                monitoring='Impedance check; PHQ-9 biweekly; apathy scale; C-SSRS; skin integrity',
                expected_response='PHQ-9 >=50% reduction; improved motivation',
                evidence_status='Moderate — DLPFC tDCS for depression well-supported; emerging PD-depression data',
                cautions='OFF-LABEL. Screen for psychosis (PD hallucinations). Suicidality monitoring mandatory.',
                when_to_escalate='If <20% improvement, add CES or taVNS; consider antidepressant adjustment',
                when_to_stop_modify='Stop if psychosis, mania, or skin lesion',
                clinical_notes_extended='Left DLPFC anodal is the best-evidenced depression montage. PD depression has similar prefrontal hypoactivation.',
                key_citations=['Brunoni et al., 2017'],
            ),
            ProtocolEntry(
                ...
                # Extended protocol fields
                protocol_name='TPS-PD-M1 — Motor TPS Protocol',
                clinical_objective='Improve motor function via focused acoustic stimulation of M1',
                primary_targets=['Bilateral M1'],
                secondary_targets=['Cortico-subcortical motor circuits'],
                device_name='NEUROLITH® TPS System (Storz Medical)',
                session_structure='Neuronavigation setup → Holocranial priming (2,000 pulses) → Targeted M1 bilateral stimulation (4,000 pulses) → Secondary SMA (2,000 pulses)',
                frequency='3 sessions/week',
                duration='30-45 min per session',
                intensity_dose='0.25 mJ/mm²; 5 Hz',
                montage_roi='Neuronavigation-guided M1 bilateral; scalp near C3/C4',
                laterality='Bilateral',
                treatment_course='6-9 sessions over 3 weeks; then maintenance',
                monitoring='UPDRS-III pre/post block; gait assessment; tolerability log',
                expected_response='UPDRS-III improvement; improved motor speed',
                evidence_status='Low — Koch et al. 2019 demonstrated UPDRS-III improvements; limited data',
                cautions='TPS is investigational/off-label for PD. Neuronavigation required. Doctor authorization mandatory.',
                when_to_escalate='If no improvement, add tDCS M1 bilateral protocol',
                when_to_stop_modify='Stop if persistent headache or motor worsening',
                clinical_notes_extended='OFF-LABEL. Koch et al. 2019 demonstrated significant UPDRS-III improvements with NEUROLITH in early PD.',
                key_citations=['Koch et al., 2019'],
            ),
            ProtocolEntry(
                ...
                # Extended protocol fields
                protocol_name='TPS-PD-TREMOR — Tremor TPS Protocol',
                clinical_objective='Modulate tremor-generating cerebellar-thalamocortical circuits via TPS',
                primary_targets=['M1 contralateral to dominant tremor'],
                secondary_targets=['Cerebellar cortex'],
                device_name='NEUROLITH® TPS System (Storz Medical)',
                session_structure='Neuronavigation setup → Targeted M1 contralateral stimulation (4,000 pulses) → Optional cerebellar (2,000 pulses)',
                frequency='3 sessions/week',
                duration='30-45 min per session',
                intensity_dose='0.25 mJ/mm²; 5 Hz',
                montage_roi='Neuronavigation-guided M1 contralateral to tremor; optional cerebellar',
                laterality='Contralateral to dominant tremor',
                treatment_course='6 sessions over 2 weeks',
                monitoring='Tremor rating; UPDRS tremor items; tolerability log',
                expected_response='Reduced tremor amplitude',
                evidence_status='Low — tremor-specific TPS evidence is very limited',
                cautions='TPS is investigational/off-label for PD tremor. OFF-LABEL.',
                when_to_escalate='If no improvement, consider DBS referral for tremor-dominant PD',
                when_to_stop_modify='Stop if tremor worsening or headache',
                clinical_notes_extended='Targets cerebellar-thalamocortical pathway. DBS remains gold standard for refractory tremor.',
                key_citations=['Koch et al., 2019'],
            ),
            ProtocolEntry(
                ...
                # Extended protocol fields
                protocol_name='TDCS-PD-FATIGUE — Fatigue Protocol',
                clinical_objective='Reduce PD-related fatigue via left DLPFC anodal stimulation',
                primary_targets=['Left DLPFC (F3)'],
                secondary_targets=['Executive-attentional circuits'],
                device_name='Newronika HDCkit (2-channel)',
                ...
                treatment_course='10-15 sessions over 3 weeks',
                monitoring='Impedance check; FSS/MFIS fatigue scales; skin integrity',
                expected_response='Reduced subjective fatigue; improved energy levels',
                evidence_status='Low — rationale from CEN hypoactivation in PD fatigue; limited tDCS data',
                cautions='OFF-LABEL. Rule out depression as primary cause of fatigue.',
                when_to_escalate='If <20% improvement, add CES; assess for sleep disorder',
                when_to_stop_modify='Stop if skin lesion or fatigue worsening',
                clinical_notes_extended='PD fatigue (40-60% prevalence) involves CEN hypoactivation. Rule out sleep apnea, depression.',
                key_citations=['Fregni et al., 2006'],
            ),
            ProtocolEntry(
                ...
                evidence_level=EvidenceLevel.LOW, off_label=True, session_count=15,
                # Extended protocol fields
                protocol_name='TDCS-PD-BALANCE — Balance & Posture Protocol',
                clinical_objective='Improve postural stability via SMA/cerebellar tDCS',
                primary_targets=['SMA (Fz)'],
                secondary_targets=['Cerebellar reference'],
                device_name='Newronika HDCkit (2-channel)',
                session_structure='Anode (+): Fz (SMA midline). Cathode (-): Inion/Oz (cerebellar reference). 2 mA, 20 min.',
                frequency='1 session/day, 5 days/week',
                duration='20 min per session',
                intensity_dose='2 mA',
                montage_roi='Anode (+): Fz (SMA midline). Cathode (-): Inion/Oz (cerebellar reference)',
                laterality='Midline',
                treatment_course='10-15 sessions over 3 weeks',
                monitoring='Impedance check; BBS (Berg Balance Scale); TUG; falls diary; skin integrity',
                expected_response='Improved postural stability and reduced falls',
                evidence_status='Low — SMA-cerebellar tDCS for balance in PD is investigational',
                cautions='OFF-LABEL. Combine with balance physiotherapy. Fall risk assessment mandatory.',
                when_to_escalate='If no improvement, consider TPS SMA protocol',
                when_to_stop_modify='Stop if falls increase or skin lesion',
                clinical_notes_extended='Postural instability involves SMA hypoactivation and impaired cerebellar compensation.',
                key_citations=['Kaski et al., 2014'],
            ),
            ProtocolEntry(
                ...
                # Extended protocol fields
                protocol_name='TDCS-PD-DYSK — ON/OFF Dyskinesia Protocol',
                clinical_objective='Reduce levodopa-induced dyskinesia via cathodal SMA inhibition',
                primary_targets=['SMA (Fz) — cathodal'],
                secondary_targets=['M1 (C3/C4) — anodal'],
                device_name='Newronika HDCkit (2-channel)',
                session_structure='Anode (+): C3/C4 (bilateral M1). Cathode (-): Fz (SMA — cathodal to reduce hyperkinetic). 1.5 mA, 20 min.',
                frequency='1 session/day, 5 days/week',
                duration='20 min per session',
                intensity_dose='1.5 mA (lower intensity for safety)',
                montage_roi='Anode (+): C3/C4 (bilateral M1). Cathode (-): Fz (SMA cathodal)',
                laterality='Bilateral/midline',
                treatment_course='10 sessions over 2 weeks',
                monitoring='Impedance check; dyskinesia rating scale; UPDRS-IV; on/off diary; skin integrity',
                expected_response='Reduced dyskinesia severity during ON-state',
                evidence_status='Low — cathodal SMA for dyskinesia is investigational; limited pilot data',
                cautions='OFF-LABEL. Lower intensity (1.5 mA) for safety. Monitor dyskinesia severity closely. Only for patients with troublesome dyskinesia.',
                when_to_escalate='If no improvement, review levodopa dose with neurologist; consider DBS referral',
                when_to_stop_modify='Stop if dyskinesia worsening, motor function decline, or skin lesion',
                clinical_notes_extended='Use only in patients with troublesome dyskinesia. Monitor carefully. Cathodal SMA reduces hyperkinetic output.',
                key_citations=['Fregni et al., 2006'],
            ),
            ProtocolEntry(
                ...
                evidence_level=EvidenceLevel.MEDIUM, off_label=True, session_count=15,
                # Extended protocol fields
                protocol_name='TDCS-PD-LEARN — Motor Learning & Skill Acquisition',
                clinical_objective='Enhance motor learning and skill retention via concurrent tDCS and physiotherapy',
                primary_targets=['M1 (C3) + SMA (Fz)'],
                secondary_targets=['Motor learning circuits'],
                device_name='Newronika HDCkit (2-channel)',
                session_structure='Anode (+): C3 + Fz (M1 + SMA). Cathode (-): Fp2 (R supraorbital). 2 mA, 20 min. During active physiotherapy motor practice.',
                frequency='1 session/day, 5 days/week',
                duration='20 min per session',
                intensity_dose='2 mA',
                montage_roi='Anode (+): C3 (L-M1), Fz (SMA). Cathode (-): Fp2',
                laterality='Left M1 + midline SMA',
                treatment_course='10-15 sessions over 3 weeks',
                monitoring='Impedance check; motor task performance; grip strength; skin integrity',
                expected_response='Enhanced motor skill acquisition and retention',
                evidence_status='Moderate — tDCS during motor practice enhances learning via LTP facilitation',
                cautions='OFF-LABEL. Must be delivered during active physiotherapy. Optimal during motor practice.',
                when_to_escalate='If <20% improvement, adjust physiotherapy tasks',
                when_to_stop_modify='Stop if skin lesion or motor worsening',
                clinical_notes_extended='tDCS during active motor practice enhances LTP-dependent motor learning. Deliver during physiotherapy sessions.',
                key_citations=['Kaski et al., 2014'],
            ),
            ProtocolEntry(
                ...
                evidence_level=EvidenceLevel.LOW, off_label=True, session_count=9,
                # Extended protocol fields
                protocol_name='TPS-PD-COG — Cognition TPS Protocol',
                clinical_objective='Enhance executive function via TPS targeting DLPFC at depth',
                primary_targets=['Bilateral DLPFC'],
                secondary_targets=['Frontostriatal circuits'],
                device_name='NEUROLITH® TPS System (Storz Medical)',
                ...
                frequency='3 sessions/week',
                duration='30-45 min per session',
                intensity_dose='0.20 mJ/mm²; 5 Hz',
                montage_roi='Neuronavigation-guided DLPFC bilateral; scalp near F3/F4',
                laterality='Bilateral',
                treatment_course='6-9 sessions over 3 weeks',
                monitoring='MoCA pre/post block; executive tests; tolerability log',
                expected_response='Improved cognitive function',
                evidence_status='Low — exploratory; extrapolated from AD TPS evidence',
                cautions='TPS is investigational/off-label for PD cognition. Neuronavigation recommended.',
                when_to_escalate='If no improvement, add tDCS DLPFC protocol',
                when_to_stop_modify='Stop if confusion or headache',
                clinical_notes_extended="Exploratory application based on Alzheimer's TPS evidence. TPS reaches deeper layers than surface tDCS.",
                key_citations=['Koch et al., 2019'],
            ),
            ProtocolEntry(
                ...
                evidence_level=EvidenceLevel.LOW, off_label=True, session_count=9,
                # Extended protocol fields
                protocol_name='TPS-PD-SMA — Gait & SMA TPS Protocol',
                clinical_objective='Modulate SMA at depth for gait initiation and FOG',
                primary_targets=['SMA midline'],
                secondary_targets=['Motor planning circuits'],
                device_name='NEUROLITH® TPS System (Storz Medical)',
                session_structure='Neuronavigation setup → Targeted SMA stimulation (4,000 pulses) → Secondary M1 (2,000 pulses)',
                frequency='3 sessions/week',
                duration='30-45 min per session',
                intensity_dose='0.25 mJ/mm²; 5 Hz',
                montage_roi='Neuronavigation-guided SMA midline; scalp near Fz/FCz',
                laterality='Midline',
                treatment_course='6-9 sessions over 3 weeks',
                monitoring='FOG-Q; gait assessment; tolerability log',
                expected_response='Reduced FOG episodes; improved gait initiation',
                evidence_status='Low — SMA TPS for PD gait is investigational',
                cautions='TPS is investigational/off-label for PD gait. Neuronavigation recommended.',
                when_to_escalate='If no improvement, add tDCS SMA+M1 gait protocol',
                when_to_stop_modify='Stop if gait worsening or headache',
                clinical_notes_extended='SMA hypoactivation contributes to FOG. TPS may reach deeper SMA regions than tDCS.',
                key_citations=['Koch et al., 2019'],
            ),
            ProtocolEntry(
                ...
                # Extended protocol fields
                protocol_name='TPS-PD-DEP — Depression & ACC TPS Protocol',
                clinical_objective='Modulate limbic-affective circuitry for PD depression and apathy',
                primary_targets=['ACC / medial PFC'],
                secondary_targets=['Subgenual ACC'],
                device_name='NEUROLITH® TPS System (Storz Medical)',
                session_structure='Neuronavigation setup → Targeted ACC/mPFC stimulation (4,000 pulses) → Secondary DLPFC (2,000 pulses)',
                frequency='3 sessions/week',
                duration='30-45 min per session',
                intensity_dose='0.20 mJ/mm²; 5 Hz',
                montage_roi='Neuronavigation-guided ACC/mPFC; scalp near Fz',
                laterality='Midline',
                treatment_course='6 sessions over 2 weeks',
                monitoring='PHQ-9; apathy scale; tolerability log',
                expected_response='Mood improvement and reduced apathy',
                evidence_status='Very low — EXPLORATORY. Limited evidence for ACC TPS in PD depression.',
                cautions='TPS is investigational/off-label for PD depression. EXPLORATORY. Doctor authorization and research consent required.',
                when_to_escalate='If no improvement, review antidepressant management',
                when_to_stop_modify='Stop if mood worsening or headache',
                clinical_notes_extended='EXPLORATORY. Limited evidence. Deep ACC stimulation may modulate subgenual region not reachable by tDCS.',
                key_citations=['Koch et al., 2019'],
            ),
            ProtocolEntry(
                ...
                # Extended protocol fields
                protocol_name='CES-PD — Alpha-Stim Anxiety/Sleep Adjunct',
                clinical_objective='Address anxiety and sleep disturbance in PD as adjunct to motor protocols',
                ...
                laterality='Bilateral',
                treatment_course='Daily or alternate day during treatment block',
                monitoring='GAD-7; PSQI; sleep diary; skin integrity',
                ...
                cautions='Adjunctive only — not primary PD treatment.',
                when_to_escalate='If symptoms persist, review PD medication (sleep/anxiety side effects)',
                when_to_stop_modify='Stop if skin irritation at earlobes',
                clinical_notes_extended='Supports limbic network regulation and sleep quality. Non-pharmacological adjunct.',
        ...
        eeg_reference_table=[
            {
                ...
                "role": 'Anodal — motor function (bradykinesia, rigidity)',
                "protocols_using": ['C1', 'C7', 'C8'],
            },
            {
                "position": 'C4',
                "brain_region": 'Right Primary Motor Cortex',
                "role": 'Bilateral motor target',
                "protocols_using": ['C1', 'C7', 'C8'],
            },
            {
                "position": 'F3',
                "brain_region": 'Left DLPFC',
                "role": 'Anodal — cognition, depression, fatigue',
                "protocols_using": ['C3', 'C4', 'C5'],
            },
            {
                "position": 'Fz',
                "brain_region": 'SMA / Midline Frontal',
                "role": 'Anodal — gait, balance, SMA activation',
                "protocols_using": ['C2', 'C6', 'C8'],
            },
            {
                "position": 'FCz',
                "brain_region": 'SMA anterior',
                "role": 'SMA target for motor planning',
                "protocols_using": ['C2'],
            },
        ],
