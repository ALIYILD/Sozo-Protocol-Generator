...
def build_ocd_condition() -> ConditionSchema:
    ...
    return ConditionSchema(
        ...
        eeg_reference_table=[
            {"position": "F3", "brain_region": "Left DLPFC", "role": "Anodal reference in bilateral DLPFC montage; left CEN upregulation", "protocols_using": ["C-OCD-R"]},
            {"position": "F4", "brain_region": "Right DLPFC", "role": "PRIMARY cathodal target — inhibits hyperactive CSTC loop via right prefrontal-striatal circuit", "protocols_using": ["C-OCD-R"]},
            {"position": "Fz", "brain_region": "Midline Frontal / ACC / dACC", "role": "dACC — OCD conflict monitoring hyperactivation; 'not done' signal generation target", "protocols_using": ["C-OCD-SMA"]},
            {"position": "FC1", "brain_region": "Left Pre-SMA", "role": "Pre-SMA — key OCD target for motor compulsion inhibition (left lateralized)", "protocols_using": ["C-OCD-SMA"]},
            {"position": "FC2", "brain_region": "Right Pre-SMA", "role": "Pre-SMA — bilateral motor compulsion inhibition target", "protocols_using": ["C-OCD-SMA"]},
        ],
        ...
        protocols=[
            ProtocolEntry(
                ...
                protocol_name="C-OCD-R — Right DLPFC Cathodal OCD Inhibition Protocol",
                clinical_objective="Inhibit hyperactive CSTC loop by reducing prefrontal drive to OFC-striatal circuit",
                primary_targets=["F4 (Right DLPFC — CATHODAL / INHIBITORY)"],
                secondary_targets=["F3 (Left DLPFC — anodal) or Fp1 (left supraorbital — anodal)"],
                device_name="Newronika HDCkit (2-channel)",
                session_structure="CATHODE F4 (right DLPFC — INHIBITORY); Anode F3 or Fp1. 2 mA, 20–30 min. REVERSED from depression montage.",
                frequency="5 sessions/week",
                duration="20–30 min per session",
                intensity_dose="2.0 mA",
                montage_roi="Cathode (−): F4 (R-DLPFC — INHIBITORY). Anode (+): F3 or Fp1",
                laterality="Right cathodal dominant (REVERSED from depression)",
                treatment_course="10–15 sessions over 3 weeks",
                monitoring="Y-BOCS at Week 2 and Week 4; OCI-R weekly; PHQ-9 every session; skin integrity",
                expected_response="≥35% Y-BOCS reduction; reduced compulsive behavior frequency; improved daily function",
                evidence_status="Medium — Mondino et al. 2015 (Brain Stimulation); Bation et al. 2016 pilot RCT",
                cautions="IMPORTANT: cathode at F4 — REVERSED polarity from depression montage. Verify electrode placement.",
                when_to_escalate="If no Y-BOCS improvement after 10 sessions, switch to SMA cathodal protocol",
                when_to_stop_modify="Standard tDCS stop criteria; monitor for paradoxical OCD exacerbation in early sessions",
                clinical_notes_extended="Coordinate with treating psychiatrist re SSRI optimization. Brief psychoeducation about possible initial symptom fluctuation before improvement.",
                key_citations=["Mondino et al., 2015", "Bation et al., 2016", "Goodman et al., 1989"],
            ),
            ProtocolEntry(
                ...
                # -- SOZO Protocol Handbook fields --
                protocol_name="C-OCD-SMA — SMA Cathodal Motor Compulsion Inhibition Protocol",
                clinical_objective="Directly reduce motor compulsion generation via SMA/pre-SMA cathodal inhibition",
                primary_targets=["Cz (SMA / vertex approximation — cathodal)"],
                secondary_targets=["Iz (inion) or shoulder reference (anodal)"],
                device_name="Newronika HDCkit (2-channel)",
                session_structure="Cathode Cz (SMA); Anode Iz or shoulder. 1.5–2.0 mA, 20 min.",
                frequency="5 sessions/week",
                duration="20 min per session",
                intensity_dose="1.5–2.0 mA",
                montage_roi="Cathode (−): Cz (SMA). Anode (+): Iz or shoulder",
                laterality="Midline",
                treatment_course="10–15 sessions over 3 weeks",
                monitoring="Y-BOCS compulsion subscale; OCI-R washing/checking/ordering subscales; motor compulsion frequency log",
                expected_response="Reduced motor compulsion frequency; decreased 'just right' urges",
                evidence_status="Low — Grassi et al. 2018 augmented SMA inhibition in OCD; limited sample size",
                cautions="For motor compulsion-predominant phenotype; cathodal Cz placement requires midline precision",
                when_to_escalate="If no compulsion reduction, switch to or combine with DLPFC cathodal protocol",
                when_to_stop_modify="Standard tDCS stop criteria",
                clinical_notes_extended="Best for symmetry/ordering and checking phenotypes with prominent motor rituals. SMA is the motor compulsion generator.",
                key_citations=["Grassi et al., 2018"],
            ),
            ProtocolEntry(
                ...
                # -- SOZO Protocol Handbook fields --
                protocol_name="TAVNS-OCD — taVNS Anxiety & Limbic Adjunct for OCD",
                clinical_objective="Modulate limbic anxiety and noradrenergic tone as adjunct for OCD with anxiety/mood comorbidity",
                primary_targets=["Auricular branch of vagus nerve → NTS → LC noradrenergic pathway"],
                secondary_targets=["Limbic network (amygdala modulation)", "SN anxiety reduction"],
                ...
                treatment_course="Daily for duration of tDCS block",
                monitoring="GAD-7; PHQ-9; heart rate; obsessional anxiety self-report",
                expected_response="Reduced obsession-related anxiety, improved mood, enhanced limbic regulation",
                evidence_status="Low — no dedicated OCD RCT; rationale from anxiety/depression vagal modulation literature",
                cautions="Investigational in OCD; adjunct only; no published OCD-specific data",
                when_to_escalate="If anxiety not reducing, consider pharmacological anxiolytic optimization",
                when_to_stop_modify="Stop if vagal side effects (bradycardia, cough)",
                clinical_notes_extended="For intrusive thoughts and contamination phenotypes with prominent anxiety. Investigational — no published OCD RCT.",
        ...
        platoscience_variants=[
            PlatoScienceVariant(
                variant_id="C-OCD-R-PS", protocol_id="C-OCD-R",
                label="PlatoScience Right DLPFC Cathodal — OCD Inhibition Protocol",
                parameters={"program": "Think", "electrode_config": "F3 (left DLPFC) or Fp1 (left supraorbital)", "intensity": "2.0 mA", "duration": "20-30 min", "ramp": "30 sec"},
                notes="Maps to C-OCD-R protocol. PlatoScience Think program.",
            ),
            PlatoScienceVariant(
                variant_id="C-OCD-SMA-PS", protocol_id="C-OCD-SMA",
                label="PlatoScience SMA Cathodal — Motor Compulsion Inhibition",
                parameters={"program": "Focus", "electrode_config": "Iz (inion) or shoulder reference", "intensity": "1.5-2.0 mA", "duration": "20 min", "ramp": "30 sec"},
                notes="Maps to C-OCD-SMA protocol. PlatoScience Focus program.",
            ),
        ],
