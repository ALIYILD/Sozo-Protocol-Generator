    ...
    return ConditionSchema(
        ...
        protocols=[
            ProtocolEntry(
                ...
                protocol_name='CES-INS — Alpha-Stim Insomnia Protocol',
                clinical_objective='Improve sleep onset, quality, and duration via CES alpha-wave entrainment',
                primary_targets=['Bilateral earlobe electrodes'],
                secondary_targets=['Cortical arousal modulation'],
                device_name='Alpha-Stim® AID',
                session_structure='Bilateral earlobe clip electrodes; 0.5 Hz, 100-300 µA; 40-60 min; 1-2 hours before bedtime',
                frequency='Daily, 1-2 hours before bedtime',
                duration='40-60 min per session',
                intensity_dose='100-300 µA at 0.5 Hz',
                montage_roi='Bilateral earlobe clip electrodes',
                laterality='Bilateral',
                treatment_course='Daily for 4-8 weeks; then as-needed maintenance',
                monitoring='PSQI biweekly; ISI; sleep diary; skin integrity',
                expected_response='Improved sleep onset latency and quality within 1-2 weeks',
                evidence_status='Moderate — CES FDA-cleared for insomnia; systematic review evidence',
                cautions='Adjunctive only — not primary treatment for severe insomnia. CBT-I remains first-line. Use 1-2 hours before bedtime.',
                when_to_escalate='If insomnia persists after 4 weeks, add tDCS or slow oscillation protocol',
                when_to_stop_modify='Stop if skin irritation or paradoxical activation',
                clinical_notes_extended='Best when combined with CBT-I (cognitive behavioral therapy for insomnia). Use in evening 1-2 hours before planned sleep.',
            ...
            ProtocolEntry(
                ...
                # Extended protocol fields
                protocol_name='TDCS-INS-COG — DLPFC Hyperarousal Modulation',
                clinical_objective='Modulate prefrontal hyperarousal and rumination contributing to insomnia',
                primary_targets=['Bilateral DLPFC (F3/F4)'],
                secondary_targets=['Prefrontal-limbic arousal circuits'],
                device_name='Newronika HDCkit (2-channel)',
                session_structure='Anode (+): F3 (L-DLPFC). Cathode (-): F4 (R-DLPFC). 1.5-2.0 mA, 20 min. Evening session preferred.',
                frequency='1 session/day, 5 days/week (evening)',
                ...
                monitoring='Impedance check; PSQI biweekly; ISI; sleep diary; skin integrity',
                expected_response='Reduced pre-sleep rumination and improved sleep quality',
                evidence_status='Low — limited insomnia-specific tDCS data; rationale from arousal/rumination neuroimaging',
                cautions='OFF-LABEL. Evening sessions recommended (not morning). Monitor for paradoxical activation.',
                when_to_escalate='If <20% improvement after 10 sessions, add CES or consider slow oscillation protocol',
                when_to_stop_modify='Stop if burning, skin lesion, or paradoxical insomnia worsening',
                clinical_notes_extended='Combine with CBT-I for best outcomes. Evening session timing important for insomnia.',
                key_citations=['Frase et al., 2019'],
            ),
            ProtocolEntry(
                ...
                # Extended protocol fields
                protocol_name='TDCS-INS-SO — Slow Oscillation Enhancement Protocol',
                clinical_objective='Enhance slow-wave sleep via frontal slow oscillation tDCS during sleep onset',
                primary_targets=['Frontal midline (Fz)'],
                secondary_targets=['Bilateral prefrontal slow oscillation generators'],
                device_name='Newronika HDCkit (2-channel)',
                session_structure='Oscillating tDCS at 0.75 Hz over Fz during early sleep. Specialized protocol requiring polysomnography.',
                frequency='During sleep onset, as scheduled',
                duration='Delivered during early NREM sleep (variable)',
                intensity_dose='0.75 Hz oscillating tDCS; 0.26 mA peak',
                montage_roi='Anode (+): Fz (midline frontal). Cathode (-): bilateral mastoids',
                laterality='Midline',
                treatment_course='Research protocol — varies by study design',
                monitoring='Polysomnography; sleep diary; PSQI',
                expected_response='Enhanced slow-wave activity and sleep consolidation',
                evidence_status='Low — research-level; Marshall et al. 2006 foundational study; limited clinical application',
                cautions='Research protocol only. Requires polysomnography setup. Not routine clinical use.',
                when_to_escalate='N/A — research protocol',
                when_to_stop_modify='Follow research protocol stopping rules',
                clinical_notes_extended='Foundational research by Marshall et al. 2006. Not yet translated to routine clinical use.',
                key_citations=['Marshall et al., 2006', 'Frase et al., 2019'],
            ),
        ],
        ...
        platoscience_variants=[
            PlatoScienceVariant(
                variant_id="TDCS-INS-SO-PS", protocol_id="TDCS-INS-SO",
                label="PlatoScience Slow-Oscillation tDCS — Sleep Maintenance (INVESTIGATIONAL)",
                parameters={"program": "Relax", "electrode_config": "Per protocol", "intensity": "2.0 mA", "duration": "20 min", "ramp": "30 sec"},
                notes="Maps to TDCS-INS-SO protocol. PlatoScience Relax program.",
            ),
        ],
        ...
        eeg_reference_table=[
            {
                ...
                "role": 'Anodal — prefrontal hyperarousal modulation',
                "protocols_using": ['C-INS-COG'],
            },
            {
                "position": 'F4',
            ...
            },
            {
                ...
                "role": 'Slow oscillation target',
                "protocols_using": ['TDCS-INS-SO'],
            },
        ],
