    ...
    return ConditionSchema(
        ...
        protocols=[
            ProtocolEntry(
                ...
                protocol_name='VNS-TRD — LivaNova SenTiva Standard Titration Protocol',
                clinical_objective='Chronic vagus nerve stimulation for treatment-resistant depression via implanted device',
                primary_targets=['Left cervical vagus nerve (C3-C5)'],
                secondary_targets=['NTS → LC → limbic/prefrontal modulation'],
                device_name='LivaNova SenTiva (Model 1000) IPG',
                session_structure='Continuous cycling: 30s ON / 5min OFF. Output 0.25-1.75 mA. Titration over 12 months. Patient magnet for on-demand burst.',
                frequency='Continuous cycling (30s ON / 5min OFF); ~9% duty cycle',
                duration='Continuous — chronic implanted therapy',
                intensity_dose='0.25-1.75 mA; 20 Hz; 250-500 µs pulse width',
                montage_roi='Left cervical vagus nerve; lead tunnelled from left infraclavicular IPG pocket',
                laterality='Left cervical',
                treatment_course='Chronic continuous; 12-month RCT phase; device longevity ~11 years',
                monitoring='MADRS monthly; voice quality; dysphagia screening; device impedance; programming logs',
                expected_response='Response accumulates over 6-12 months; 67.6% response at 12 months (RECOVER trial)',
                evidence_status='Highest — FDA-approved; RECOVER trial (Aaronson 2022): 67.6% vs 40.2% BMT at 12 months',
                cautions='Surgical implant procedure. Slow titration required. Do not assess treatment failure before Month 9 at therapeutic dose. Voice hoarseness common.',
                when_to_escalate='If subtherapeutic at max current (1.75 mA), increase pulse width to 500 µs',
                when_to_stop_modify='Device can be turned off if intolerable AEs; explant rarely needed',
                clinical_notes_extended='VNS is chronic continuous therapy. Response accumulates slowly over 6-12 months. Patience required.',
                key_citations=['Aaronson et al., 2022', 'Rush et al., 2005'],
            ),
            ProtocolEntry(
                ...
                # Extended protocol fields
                protocol_name='VNS-TRD-ANX — Modified Slow Titration for Anxious TRD',
                clinical_objective='Adapted VNS titration protocol for anxious TRD phenotype with extended intervals',
                primary_targets=['Left cervical vagus nerve (C3-C5)'],
                secondary_targets=['Limbic-SN anxiolytic pathway'],
                device_name='LivaNova SenTiva (Model 1000) IPG',
                session_structure='Same hardware as VNS-TRD. Extended titration: 4-week intervals instead of 2-week. Lower ceiling (1.25 mA).',
                frequency='Continuous cycling (30s ON / 5min OFF)',
                duration='Continuous — chronic implanted therapy',
                intensity_dose='0.25-1.25 mA (lower ceiling); 20 Hz; 250 µs',
                montage_roi='Left cervical vagus nerve',
                laterality='Left cervical',
                treatment_course='Extended titration over 18+ weeks; then chronic maintenance',
                monitoring='HAM-A at every visit; MADRS monthly; pause titration if HAM-A increases >5 points',
                expected_response='Slower but sustained anti-anxiety and antidepressant effect at 6-12 months',
                evidence_status='Moderate — based on D-23 registry subgroup analysis and expert consensus',
                cautions='Initial VNS can transiently increase anxiety. Extended titration intervals (4 weeks) mitigate this. Maintain SSRI/SNRI throughout.',
                when_to_escalate='If HAM-A stable, may cautiously increase beyond 1.25 mA',
                when_to_stop_modify='Pause titration if HAM-A increases >5 points; hold current dose',
                clinical_notes_extended='Anxious TRD sub-protocol. Transient anxiety augmentation at initiation does not predict long-term outcome.',
                key_citations=['Aaronson et al., 2022'],
            ),
        ],
        ...
        eeg_reference_table=[
            {
                ...
                "role": 'Concurrent tDCS target for VNS augmentation',
                "protocols_using": [],
            },
            {
                "position": 'F4',
                "brain_region": 'Right DLPFC',
                "role": 'Cathodal reference or anxious TRD target',
                "protocols_using": [],
            },
        ],
