    ...
    return ConditionSchema(
        ...
        protocols=[
            ProtocolEntry(
                ...
                protocol_name='TDCS-MS-MOTOR — M1 Bilateral Motor Function Protocol',
                clinical_objective='Improve motor function and reduce spasticity via bilateral M1 anodal stimulation',
                primary_targets=['Bilateral M1 (C3/C4)'],
                secondary_targets=['Corticospinal tract'],
                ...
                monitoring='Impedance check; EDSS component scores; 25-foot walk test; grip strength; skin integrity',
                expected_response='Improved walking speed and upper limb function',
                evidence_status='Moderate — multiple tDCS MS motor studies; meta-analyses supportive',
                cautions='OFF-LABEL. Avoid during MS relapse. Concurrent physiotherapy recommended.',
                when_to_escalate='If <20% improvement after 10 sessions, add taVNS for fatigue or adjust targeting',
                when_to_stop_modify='Stop if relapse signs, skin lesion, or neurological worsening',
                clinical_notes_extended='Concurrent physiotherapy/rehabilitation during stimulation enhances motor outcomes.',
                key_citations=['Mori et al., 2013', 'Meesen et al., 2014'],
            ),
            ProtocolEntry(
                ...
                # Extended protocol fields
                protocol_name='TDCS-MS-COG — DLPFC Cognitive & Fatigue Protocol',
                clinical_objective='Improve cognitive function and reduce MS-related fatigue via DLPFC stimulation',
                primary_targets=['Bilateral DLPFC (F3/F4)'],
                secondary_targets=['Prefrontal executive circuits'],
                device_name='Newronika HDCkit (2-channel)',
                session_structure='Anode (+): F3 (L-DLPFC). Cathode (-): F4 or Fp2. 2 mA, 20 min.',
                frequency='1 session/day, 5 days/week',
                duration='20 min per session',
                intensity_dose='2 mA',
                ...
                treatment_course='10-15 sessions over 2-3 weeks',
                monitoring='Impedance check; SDMT (Symbol Digit); fatigue scales (FSS/MFIS); skin integrity',
                expected_response='Improved processing speed and reduced fatigue by Week 2-3',
                evidence_status='Moderate — DLPFC tDCS for MS cognition and fatigue has supporting pilot data',
                cautions='OFF-LABEL. Avoid during MS relapse. Monitor for fatigue exacerbation.',
                when_to_escalate='If <20% improvement after 10 sessions, add CES for sleep/mood; reassess with neurologist',
                when_to_stop_modify='Stop if relapse signs, skin lesion, or cognitive worsening',
                clinical_notes_extended='MS fatigue is the most common MS symptom. DLPFC tDCS targets both cognitive impairment and fatigue.',
                key_citations=['Mori et al., 2013'],
            ),
            ProtocolEntry(
                ...
                # Extended protocol fields
                protocol_name='TAVNS-MS — Fatigue Adjunct Protocol',
                clinical_objective='Reduce MS-related fatigue via vagal autonomic modulation',
                primary_targets=['Left cymba conchae (auricular CN X)'],
                secondary_targets=['Autonomic rebalancing', 'Anti-inflammatory vagal pathway'],
                device_name='NEMOS (tVNS Technologies) or Parasym',
                session_structure='Electrode on left cymba conchae; 25 Hz, 250 µs; below pain threshold; 30 min',
                ...
                laterality='Left ear',
                treatment_course='Daily for 8-12 weeks',
                monitoring='Heart rate; FSS/MFIS fatigue scales; skin integrity; tolerability',
                expected_response='Reduced fatigue and improved autonomic balance',
                evidence_status='Low — emerging evidence for taVNS in MS fatigue; anti-inflammatory vagal pathway rationale',
                cautions='Left ear only. Monitor heart rate. Adjunct to disease-modifying therapy.',
                when_to_escalate='If no improvement after 8 weeks, reassess fatigue etiology',
                when_to_stop_modify='Stop if bradycardia or ear pain',
                clinical_notes_extended='taVNS anti-inflammatory vagal pathway may have disease-modifying potential in MS (speculative).',
            ...
            ProtocolEntry(
                ...
                # Extended protocol fields
                protocol_name='CES-MS — Sleep, Depression & Anxiety Adjunct',
                clinical_objective='Address sleep, depression, and anxiety comorbidities in MS',
                primary_targets=['Bilateral earlobe electrodes'],
                secondary_targets=['Limbic modulation'],
                ...
                laterality='Bilateral',
                treatment_course='Daily for treatment block; then as-needed',
                monitoring='PHQ-9; GAD-7; PSQI; skin integrity',
                expected_response='Improved sleep and reduced anxiety/depression within 1-2 weeks',
                evidence_status='Moderate — CES FDA-cleared for anxiety, depression, insomnia',
                cautions='Adjunctive only. Not primary MS treatment.',
                when_to_escalate='If symptoms persist, review MS-specific pharmacotherapy',
                when_to_stop_modify='Stop if skin irritation',
                clinical_notes_extended='Non-pharmacological adjunct for the prevalent mood/sleep comorbidities in MS.',
        ...
        platoscience_variants=[
            PlatoScienceVariant(
                variant_id="C-MS-MOTOR-PS", protocol_id="C-MS-MOTOR",
                label="PlatoScience Motor Function — M1 Bilateral tDCS",
                parameters={"program": "Focus", "electrode_config": "C3 + C4 (bilateral M1)", "intensity": "2.0 mA", "duration": "20 min", "ramp": "30 sec"},
                notes="Maps to C-MS-MOTOR protocol. PlatoScience Focus program.",
            ),
            PlatoScienceVariant(
                variant_id="C-MS-COG-PS", protocol_id="C-MS-COG",
                label="PlatoScience Cognitive & Fatigue — DLPFC Bilateral tDCS",
                parameters={"program": "Think", "electrode_config": "F3 + F4 (bilateral DLPFC)", "intensity": "2.0 mA", "duration": "20 min", "ramp": "30 sec"},
                notes="Maps to C-MS-COG protocol. PlatoScience Think program.",
            ),
        ],
        ...
        eeg_reference_table=[
            {
                ...
                "role": 'Anodal — cognitive/fatigue target',
                "protocols_using": ['C-MS-COG'],
            },
            {
                "position": 'F4',
            ...
            },
            {
                ...
                "role": 'Anodal — motor function target',
                "protocols_using": ['C-MS-MOTOR'],
            },
            {
                "position": 'C4',
