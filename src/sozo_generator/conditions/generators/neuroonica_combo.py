    ...
    return ConditionSchema(
        ...
        protocols=[
            ProtocolEntry(
                ...
                protocol_name='NNO-ADHD — Newronika EEG-guided ADHD Protocol',
                clinical_objective='Enhance attention and executive function via EEG-guided bilateral DLPFC tDCS with real-time monitoring',
                primary_targets=['Bilateral DLPFC (F3/F4)'],
                secondary_targets=['Frontoparietal attention network'],
                device_name='Newronika HDCkit (2-channel) with EEG monitoring',
                session_structure='EEG baseline → Anode (+): F3 (L-DLPFC). Cathode (-): F4 (R-DLPFC). 2 mA, 20 min. Real-time EEG monitoring.',
                frequency='1 session/day, 5 days/week',
                duration='20 min per session',
                intensity_dose='2 mA per channel',
                montage_roi='Anode (+): F3 (L-DLPFC). Cathode (-): F4 (R-DLPFC). EEG monitoring channels.',
                laterality='Left anodal / Right cathodal',
                treatment_course='10-15 sessions over 2-3 weeks',
                monitoring='Real-time EEG; impedance check; ASRS weekly; BRIEF-2; skin integrity',
                expected_response='Improved attention and reduced impulsivity by Week 2-3',
                evidence_status='Moderate — DLPFC tDCS for ADHD supported by multiple studies; EEG-guided is novel',
                cautions='OFF-LABEL. EEG-guided approach is investigational addition to standard tDCS protocol.',
                when_to_escalate='If <20% improvement after 10 sessions, adjust EEG-guided targeting',
                when_to_stop_modify='Stop if burning, skin lesion, or neurological change',
                clinical_notes_extended='Newronika EEG-guided approach allows real-time monitoring of cortical response to stimulation.',
                key_citations=['Sievert et al., 2022'],
            ),
            ProtocolEntry(
                protocol_id="NNO-MDD",
                ...
                # Extended protocol fields
                protocol_name='NNO-MDD — Newronika EEG-guided Depression Protocol',
                clinical_objective='Treat MDD via EEG-guided left DLPFC anodal tDCS with real-time cortical monitoring',
                primary_targets=['Left DLPFC (F3)'],
                secondary_targets=['Prefrontal-limbic circuits'],
                device_name='Newronika HDCkit (2-channel) with EEG monitoring',
                session_structure='EEG baseline → Anode (+): F3 (L-DLPFC). Cathode (-): Fp2 or F4. 2 mA, 30 min. Real-time EEG.',
                frequency='1 session/day, 5 days/week',
                duration='30 min per session',
                intensity_dose='2 mA',
                montage_roi='Anode (+): F3 (L-DLPFC). Cathode (-): Fp2 or F4. EEG monitoring channels.',
                laterality='Left anodal',
                treatment_course='20 sessions over 4 weeks; then taper',
                monitoring='Real-time EEG; impedance check; PHQ-9 weekly; C-SSRS; skin integrity',
                expected_response='PHQ-9 >=50% reduction by Week 4',
                evidence_status='Moderate — standard DLPFC tDCS for MDD well-supported; EEG-guided adds monitoring',
                cautions='Screen for bipolar disorder. Suicidality monitoring mandatory. EEG-guided is novel addition.',
                when_to_escalate='If <20% improvement after 10 sessions, adjust EEG-guided targeting or switch protocol',
                when_to_stop_modify='Stop if suicidal ideation increase, skin lesion, or manic switch',
                clinical_notes_extended='EEG monitoring enables real-time verification of cortical engagement during stimulation.',
            ...
            ProtocolEntry(
                ...
                session_count=10,
                # Extended protocol fields
                protocol_name='NNO-COG — Newronika EEG-guided Cognitive Enhancement',
                clinical_objective='Enhance cognitive function via EEG-guided DLPFC tDCS for MCI or cognitive decline',
                primary_targets=['Left DLPFC (F3)'],
                secondary_targets=['Frontoparietal network'],
                device_name='Newronika HDCkit (2-channel) with EEG monitoring',
                session_structure='EEG baseline → Anode (+): F3 (L-DLPFC). Cathode (-): right shoulder or Fp2. 2 mA, 20 min. Real-time EEG.',
                frequency='1 session/day, 5 days/week',
                duration='20 min per session',
                intensity_dose='2 mA',
                montage_roi='Anode (+): F3 (L-DLPFC). Cathode (-): right shoulder or Fp2. EEG monitoring.',
                laterality='Left anodal',
                treatment_course='10-15 sessions over 2-3 weeks',
                monitoring='Real-time EEG; impedance check; MoCA; cognitive battery; skin integrity',
                expected_response='Improved cognitive performance by Week 2-3',
                evidence_status='Moderate — DLPFC tDCS for cognition supported; EEG-guided adds monitoring dimension',
                cautions='OFF-LABEL. Concurrent cognitive training during stimulation recommended.',
                when_to_escalate='If <20% improvement after 10 sessions, consider TPS or rTMS',
                when_to_stop_modify='Stop if confusion, skin lesion, or cognitive worsening',
                clinical_notes_extended='EEG-guided approach allows personalization based on individual cortical response patterns.',
            ...
            ProtocolEntry(
                ...
                # Extended protocol fields
                protocol_name='NNO-MOTOR — Newronika EEG-guided Motor Rehabilitation',
                clinical_objective='Enhance motor recovery via EEG-guided bilateral M1 tDCS with cortical monitoring',
                primary_targets=['Bilateral M1 (C3/C4)'],
                secondary_targets=['Corticospinal tract', 'SMA'],
                device_name='Newronika HDCkit (2-channel) with EEG monitoring',
                session_structure='EEG baseline → Anode (+): C3 (affected M1). Cathode (-): C4 or contralateral supraorbital. 2 mA, 20 min. Real-time EEG.',
                frequency='1 session/day, 5 days/week',
                duration='20 min per session',
                intensity_dose='2 mA',
                montage_roi='Anode (+): C3 (affected M1). Cathode (-): C4 or contralateral supraorbital. EEG monitoring.',
                laterality='Affected hemisphere anodal',
                treatment_course='10-15 sessions over 2-3 weeks',
                monitoring='Real-time EEG; impedance check; grip strength; motor assessments; skin integrity',
                expected_response='Improved motor function in affected limb',
                evidence_status='Moderate — M1 tDCS for motor rehabilitation well-supported; EEG-guided adds monitoring',
                cautions='OFF-LABEL. Concurrent physiotherapy during stimulation essential.',
                when_to_escalate='If <20% improvement after 10 sessions, adjust EEG-guided targeting',
                when_to_stop_modify='Stop if skin lesion, motor worsening, or seizure',
                clinical_notes_extended='EEG monitoring can identify motor evoked potential changes during stimulation.',
        ...
        eeg_reference_table=[
            {
                ...
                "role": "Anodal — executive/attention/depression target",
                "protocols_using": ["NNO-ADHD", "NNO-MDD", "NNO-COG"],
            },
            {
                "position": "F4",
                "brain_region": "Right DLPFC",
                "role": "Cathodal or bilateral target",
                "protocols_using": ["NNO-ADHD", "NNO-MDD"],
            },
            {
                "position": "C3",
                "brain_region": "Left Primary Motor Cortex",
                "role": "Anodal — motor rehabilitation",
                "protocols_using": ["NNO-MOTOR"],
            },
            {
                "position": "C4",
