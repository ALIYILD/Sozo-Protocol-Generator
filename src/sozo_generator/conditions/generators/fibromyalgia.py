...
def build_fibromyalgia_condition() -> ConditionSchema:
    ...
    return ConditionSchema(
        ...
        eeg_reference_table=[
            {"position": "C3", "brain_region": "Left Primary Motor Cortex (M1)", "role": "PRIMARY anodal target — M1-PAG descending inhibitory pathway activation (Fregni 2006)", "protocols_using": ["C-FMS-M1-ANODAL"]},
            {"position": "C4", "brain_region": "Right Primary Motor Cortex (M1)", "role": "Bilateral M1 extension for bilateral pain; contralateral pain modulation", "protocols_using": ["C-FMS-M1-ANODAL"]},
            {"position": "F3", "brain_region": "Left DLPFC", "role": "Cognitive dysfunction (fibro fog); descending inhibition support; mood-comorbid phenotype target", "protocols_using": ["C-FMS-DLPFC-ANODAL"]},
            {"position": "F4", "brain_region": "Right DLPFC", "role": "Bilateral prefrontal extension; right-hemispheric anxiety modulation", "protocols_using": []},
            {"position": "Fz", "brain_region": "Midline Frontal / ACC", "role": "Affective pain component; ACC hyperactivation in fibromyalgia; pain suffering dimension", "protocols_using": []},
        ],
        ...
        protocols=[
            ProtocolEntry(
                ...
                protocol_name="C-FMS-M1-ANODAL — M1 Anodal tDCS Primary Fibromyalgia Pain Protocol",
                clinical_objective="Activate M1-PAG descending inhibitory pathway for central sensitization reversal and pain reduction",
                primary_targets=["C3 (Left M1 — contralateral to dominant hemisphere — anodal)"],
                secondary_targets=["Right shoulder (cathodal reference)"],
                device_name="Newronika HDCkit (2-channel)",
                session_structure="Anodal C3 (left M1); Cathode right shoulder. 2 mA, 20 min. Aerobic exercise after each session recommended.",
                frequency="5 sessions/week (Fregni protocol: 5 consecutive sessions minimum)",
                duration="20 min per session",
                intensity_dose="2.0 mA (start at 1.5 mA for central sensitization tolerability, titrate up)",
                montage_roi="Anode (+): C3 (Left M1). Cathode (−): right shoulder",
                laterality="Left M1 dominant",
                treatment_course="10–20 sessions over 3–5 weeks",
                monitoring="NRS pain at every session; FIQR at Week 4 and endpoint; skin integrity",
                expected_response="≥14% FIQR improvement; ≥2-point NRS reduction; cumulative effect at Week 3–4",
                evidence_status="HIGH — Fregni et al. 2006 RCT (N=32); Riberto et al. 2011 (M1 tDCS + rehabilitation); IFCN Level B",
                cautions="Start at 1.5 mA for sensitized patients; titrate to 2.0 mA over 2–3 sessions",
                when_to_escalate="If <30% NRS improvement at Week 4, add DLPFC protocol or CES for sleep",
                when_to_stop_modify="Standard tDCS stop criteria; reduce to 1.5 mA if persistent pain increase",
                clinical_notes_extended="Combine M1 tDCS with aerobic exercise immediately after each session — Riberto protocol; synergy is significant. Central sensitivity: start at 1.5 mA.",
                key_citations=["Fregni et al., 2006", "Riberto et al., 2011", "Clauw, 2014"],
            ),
            ProtocolEntry(
                ...
                # -- SOZO Protocol Handbook fields --
                protocol_name="C-FMS-DLPFC-ANODAL — L-DLPFC Anodal tDCS Fibro Fog & Mood Protocol",
                clinical_objective="Upregulate CEN for cognitive dysfunction (fibro fog) and descending pain inhibition; address comorbid depression",
                primary_targets=["F3 (Left DLPFC — anodal)"],
                secondary_targets=["Right shoulder or Fp2 (cathodal)"],
                device_name="Newronika HDCkit (2-channel)",
                session_structure="Anodal F3; Cathode right shoulder or Fp2. 1.5–2.0 mA, 20 min.",
                ...
                monitoring="PHQ-9 at every session; NRS pain; cognitive self-report (fibro fog)",
                expected_response="Improved cognitive function, reduced depression, enhanced pain coping",
                evidence_status="Medium — evidence from MDD tDCS literature; limited FMS-specific DLPFC data",
                cautions="For cognitive dysfunction and mood-comorbid phenotype; combined M1+DLPFC for dual-target approach",
                when_to_escalate="If cognitive symptoms persist, consider dual-site tDCS (M1+DLPFC)",
                when_to_stop_modify="Standard tDCS stop criteria",
                clinical_notes_extended="For mood-comorbid and cognitive-predominant FMS phenotypes. Fibro fog predicts poor engagement — explain as brain network dysfunction.",
                key_citations=["Fregni et al., 2006", "Clauw, 2014"],
            ),
            ProtocolEntry(
                ...
                # -- SOZO Protocol Handbook fields --
                protocol_name="TAVNS-FMS — taVNS Vagal Pain Modulation Adjunct",
                clinical_objective="Activate vagal-NTS ascending modulation of pain networks and descending pain inhibition",
                primary_targets=["Auricular branch of vagus nerve → NTS → pain modulation pathways"],
                secondary_targets=["Limbic network (depression/anxiety comorbidity)", "Anti-inflammatory vagal pathway"],
                ...
                frequency="Daily adjunct to tDCS block",
                duration="30 min per session",
                intensity_dose="Below pain threshold (0.5–3 mA)",
                ...
                monitoring="Pain threshold; NRS pain; heart rate; mood self-report",
                expected_response="Enhanced descending inhibition, reduced central sensitization, improved mood",
                evidence_status="Low — investigational for fibromyalgia; growing evidence for pain conditions",
                cautions="Adjunct only; no dedicated fibromyalgia RCT; informed consent required",
                when_to_escalate="If no improvement, focus on primary tDCS M1 protocol optimization",
                when_to_stop_modify="Stop if vagal side effects (bradycardia, cough)",
                clinical_notes_extended="Investigational adjunct — activates vagal anti-inflammatory mechanisms and supports descending pain inhibition.",
                key_citations=["Rong et al., 2016"],
            ),
            ProtocolEntry(
                protocol_id="CES-FMS",
                ...
                # -- SOZO Protocol Handbook fields --
                protocol_name="CES-FMS — Alpha-Stim CES Sleep & Mood Adjunct for Fibromyalgia",
                clinical_objective="Address the sleep-pain cycle: improve non-restorative sleep and reduce anxiety/mood symptoms",
                ...
                frequency="Nightly before sleep during treatment block",
                duration="40–60 min per session",
                intensity_dose="100–200 µA",
                montage_roi="Bilateral earlobe clip electrodes",
                laterality="Bilateral",
                treatment_course="Nightly for duration of tDCS block (4–5 weeks); maintenance 3×/week",
                monitoring="PSQI at Week 4 and endpoint; PHQ-9; skin integrity at earlobe sites",
                expected_response="Improved sleep quality, reduced anxiety, mood improvement — indirect pain reduction",
                evidence_status="Medium — FDA-cleared for anxiety, depression, insomnia; addresses FMS comorbidities",
                cautions="Sleep-pain cycle critical: poor sleep lowers pain threshold; CES nightly is key adjunct",
                when_to_escalate="If sleep not improving, add sleep hygiene intervention and pharmacotherapy review",
                when_to_stop_modify="Stop if skin irritation at earlobe sites",
                clinical_notes_extended="Non-restorative sleep perpetuates FMS fatigue and pain. CES nightly improves sleep quality — a powerful adjunct to tDCS pain protocols.",
