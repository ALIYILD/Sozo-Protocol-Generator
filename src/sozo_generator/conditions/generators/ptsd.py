    ...
    return ConditionSchema(
        ...
        protocols=[
            ProtocolEntry(
                ...
                protocol_name='TDCS-PTSD-R — Right DLPFC Fear Regulation Protocol',
                clinical_objective='Enhance prefrontal regulatory control over amygdala hyperreactivity and fear circuit',
                primary_targets=['Right DLPFC (F4) — anodal'],
                secondary_targets=['vmPFC-amygdala extinction pathway'],
                device_name='Newronika HDCkit (2-channel)',
                session_structure='Anode (+): F4 (R-DLPFC). Cathode (-): Fp1 (left supraorbital) or left shoulder. 2 mA, 30 min. May combine with psychotherapy exposure component.',
                frequency='1 session/day, 5 days/week',
                duration='30 min per session',
                intensity_dose='2 mA',
                montage_roi='Anode (+): F4 (R-DLPFC). Cathode (-): Fp1 (L supraorbital) or left shoulder',
                laterality='Right DLPFC anodal',
                treatment_course='10-15 sessions over 3 weeks',
                monitoring='Impedance check; PCL-5 biweekly; C-SSRS EVERY session (MANDATORY); post-session grounding check; skin integrity',
                expected_response='PCL-5 >=10-point reduction; reduced hyperarousal and intrusion symptoms',
                evidence_status='Moderate — Isserles et al. 2013 deep TMS evidence; rationale from interhemispheric model',
                cautions='MANDATORY C-SSRS at every session. Post-session grounding check. Ensure psychological support available. OFF-LABEL.',
                when_to_escalate='If <20% improvement after 10 sessions, add taVNS or switch to left DLPFC depression protocol',
                when_to_stop_modify='Stop if suicidal ideation worsens, severe dissociation, or skin lesion',
                clinical_notes_extended='May combine with psychotherapy (EMDR, PE, CPT). tDCS during initial exposure component may enhance extinction learning.',
                key_citations=['Isserles et al., 2013'],
            ),
            ProtocolEntry(
                ...
                # Extended protocol fields
                protocol_name='TDCS-PTSD-L — Left DLPFC Depression & Mood Protocol',
                clinical_objective='Treat comorbid depression and mood symptoms in PTSD via left DLPFC anodal stimulation',
                primary_targets=['Left DLPFC (F3) — anodal'],
                secondary_targets=['CEN-DMN-limbic circuit'],
                device_name='Newronika HDCkit (2-channel)',
                ...
                treatment_course='10-15 sessions over 3 weeks',
                monitoring='Impedance check; PHQ-9 biweekly; PCL-5; C-SSRS EVERY session (MANDATORY); skin integrity',
                expected_response='PHQ-9 >=50% reduction; improved mood and anhedonia',
                evidence_status='Moderate — standard MDD protocol applied to PTSD-depression comorbidity',
                cautions='C-SSRS mandatory at every session. Screen for bipolar disorder. OFF-LABEL.',
                when_to_escalate='If <20% improvement, add CES or taVNS; review antidepressant regimen',
                when_to_stop_modify='Stop if suicidal ideation increase, manic switch, or skin lesion',
                clinical_notes_extended='Standard MDD protocol adapted for PTSD-depression triad.',
            ...
            ProtocolEntry(
                ...
                # Extended protocol fields
                protocol_name='TAVNS-PTSD — Autonomic & Fear Extinction Adjunct',
                clinical_objective='Reduce amygdala hyperreactivity and restore autonomic balance via vagal modulation',
                primary_targets=['Left cymba conchae (auricular CN X)'],
                secondary_targets=['NTS-amygdala pathway', 'LC-NE extinction consolidation'],
                device_name='NEMOS (tVNS Technologies) or Parasym',
                session_structure='Electrode on left cymba conchae; 25 Hz, 250 µs; below pain threshold; 30-60 min',
                frequency='Daily adjunct during tDCS block',
                duration='30-60 min per session',
                intensity_dose='Below pain threshold (0.5-3.0 mA); 25 Hz',
                montage_roi='Left cymba conchae auricular electrode',
                laterality='Left ear',
                treatment_course='Daily during tDCS treatment block',
                monitoring='Heart rate; HRV; skin integrity; tolerability; PCL-5',
                expected_response='Improved HRV; reduced hyperarousal symptoms; enhanced extinction learning',
                evidence_status='Low — investigational in PTSD; animal model evidence for extinction enhancement; limited human data',
                cautions='Left ear only. Monitor heart rate. Investigational. Combine with exposure-based psychotherapy for extinction enhancement.',
                when_to_escalate='If no improvement, review psychotherapy engagement and medication',
                when_to_stop_modify='Stop if bradycardia, ear pain, or severe autonomic symptoms',
                clinical_notes_extended='Vagal stimulation during extinction learning may enhance consolidation (LC-NE-dependent mechanism).',
                key_citations=['Badran et al., 2022'],
            ),
            ProtocolEntry(
                ...
                # Extended protocol fields
                protocol_name='CES-PTSD — Insomnia, Nightmares & Hyperarousal Adjunct',
                clinical_objective='Address PTSD sleep disturbance, nightmares, and hyperarousal',
                primary_targets=['Bilateral earlobe electrodes'],
                secondary_targets=['Limbic arousal modulation'],
                device_name='Alpha-Stim® AID',
                session_structure='Bilateral earlobe clip electrodes; 0.5 Hz, 100-300 µA; 60 min; nightly before sleep',
                ...
                treatment_course='Nightly during treatment block; then as-needed maintenance',
                monitoring='PSQI; nightmare frequency; PCL-5 sleep items; skin integrity',
                expected_response='Improved sleep onset and quality; reduced nightmare frequency within 1-2 weeks',
                evidence_status='Moderate — CES FDA-cleared for anxiety and insomnia; limited PTSD-specific data',
                cautions='Adjunctive only — not primary PTSD treatment. Non-pharmacological sleep aid reduces medication burden.',
                when_to_escalate='If sleep symptoms persist, review prazosin or other PTSD-specific pharmacotherapy',
                when_to_stop_modify='Stop if skin irritation or paradoxical arousal',
                clinical_notes_extended='Addresses hyperarousal symptom cluster. Reduces limbic hyperarousal to facilitate sleep onset.',
        ...
        eeg_reference_table=[
            {
                ...
                "role": 'Anodal — depression/mood regulation in PTSD-DEP',
                "protocols_using": ['C-PTSD-L'],
            },
            {
                "position": 'F4',
                "brain_region": 'Right DLPFC',
                "role": 'Anodal — fear regulation and extinction enhancement',
                "protocols_using": ['C-PTSD-R'],
            },
            {
                "position": 'T3',
                "brain_region": 'Left Temporal / Amygdala proxy',
                "role": 'Amygdala-related limbic target',
                "protocols_using": [],
            },
            {
                "position": 'T4',
                "brain_region": 'Right Temporal / Amygdala proxy',
                "role": 'Amygdala-related limbic target',
                "protocols_using": [],
            },
            {
                ...
                "role": 'Medial prefrontal extinction circuit',
                "protocols_using": [],
            },
        ],
