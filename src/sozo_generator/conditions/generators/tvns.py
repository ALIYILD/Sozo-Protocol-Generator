    ...
    return ConditionSchema(
        ...
        protocols=[
            ProtocolEntry(
                ...
                protocol_name='TAVNS-CC-STD — Cymba Concha Standard Anxiety/Stress Protocol',
                clinical_objective='Reduce anxiety, stress, and subthreshold depression via auricular vagus nerve stimulation',
                primary_targets=['Left cymba concha (ABVN)'],
                secondary_targets=['NTS → LC → amygdala modulation'],
                device_name='NEMOS (tVNS Technologies) or Parasym',
                session_structure='Electrode on left cymba concha; 120 Hz, 250 µs; sensory threshold (0.5-3.0 mA); 30-60 min',
                frequency='Daily or 5x/week',
                duration='30-60 min per session',
                intensity_dose='Sensory threshold (typically 0.5-3.0 mA); 120 Hz; 250 µs',
                montage_roi='Left cymba concha auricular electrode',
                laterality='Left ear (primary); bilateral option',
                treatment_course='Daily for 4-8 weeks; then 3x/week maintenance',
                monitoring='Heart rate; GAD-7; PHQ-9; HRV; skin integrity; paresthesia threshold log',
                expected_response='Reduced anxiety and improved HRV within 2-4 weeks',
                evidence_status='Moderate — Burger et al. 2020; Hein et al. 2013; multiple tVNS RCTs',
                cautions='Left ear primary. Monitor heart rate. Optimize intensity to consistent sensory threshold.',
                when_to_escalate='If insufficient effect after 4 weeks, add tDCS DLPFC augmentation',
                when_to_stop_modify='Stop if bradycardia, ear pain, or skin irritation',
                clinical_notes_extended='Standard cymba concha tVNS protocol. Threshold may change across sessions — document at each session.',
                key_citations=['Burger et al., 2020', 'Hein et al., 2013'],
            ),
            ProtocolEntry(
                ...
                # Extended protocol fields
                protocol_name='TAVNS-MA-FDA — Monarch eTNS Mastoid Protocol',
                clinical_objective='Home-based nightly tVNS for MDD/TRD adjunct via Monarch eTNS system',
                primary_targets=['Bilateral mastoid process / ABVN'],
                secondary_targets=['LC-NA upregulation'],
                device_name='Monarch eTNS System (electroCore) — FDA Breakthrough Device',
                session_structure='Bilateral mastoid placement; 120 Hz, 250 µs; sensory threshold; 60 min nightly',
                frequency='Nightly for 8 weeks',
                duration='60 min per session',
                intensity_dose='Sensory threshold; 120 Hz; 250 µs',
                montage_roi='Bilateral mastoid process electrodes',
                laterality='Bilateral',
                treatment_course='Nightly for 8 weeks (home-based); app-supervised',
                monitoring='Monarch app adherence tracking; HAMD-17; PHQ-9; session logging',
                expected_response='HAMD-17 reduction; mood improvement over 4-8 weeks',
                evidence_status='Moderate — FDA Breakthrough Device designation; Rong et al. 2016 pilot',
                cautions='FDA Breakthrough Device designation does not constitute clearance/approval. Use within clinical trial or off-label with informed consent.',
                when_to_escalate='If insufficient response after 8 weeks, consider combination with tDCS',
                when_to_stop_modify='Stop if intolerable side effects or skin irritation',
                clinical_notes_extended='Home-based nightly protocol enables high cumulative dose. FDA Breakthrough does not mean cleared.',
            ...
            ProtocolEntry(
                ...
                session_count=20,
                # Extended protocol fields
                protocol_name='TAVNS-PREV — Preventive Stress Resilience Protocol',
                clinical_objective='Preventive vagal modulation for subclinical stress/anxiety before clinical escalation',
                primary_targets=['Left cymba concha (ABVN)'],
                secondary_targets=['Autonomic/parasympathetic upregulation'],
                device_name='Neuvana Xen, Parasym, or equivalent CE/FDA-cleared device',
                session_structure='Left cymba concha electrode; 120 Hz, 250 µs; sub-sensory to sensory threshold (0.25-1.0 mA); 30 min',
                frequency='Daily for 4 weeks, then 3x/week maintenance',
                duration='30 min per session',
                intensity_dose='Sub-sensory to sensory threshold (0.25-1.0 mA); 120 Hz',
                montage_roi='Left cymba concha auricular electrode',
                laterality='Left ear',
                treatment_course='Daily 4 weeks then 3x/week maintenance; scalable home-based',
                monitoring='PSS; GAD-7; HRV; session adherence',
                expected_response='PSS reduction >=5 points; HRV improvement; prevention of clinical escalation',
                evidence_status='Low — Clancy et al. 2014 HRV improvement in healthy adults; preventive application is novel',
                cautions='Lower intensity protocol. Sub-sensory may be adequate for non-clinical populations.',
                when_to_escalate='If stress escalates to clinical levels, upgrade to standard tVNS protocol',
                when_to_stop_modify='Stop if any adverse effects',
                clinical_notes_extended='Scalable home-based preventive model. PSS and HRV serve as early response biomarkers.',
                key_citations=['Clancy et al., 2014'],
            ),
            ProtocolEntry(
                ...
                # Extended protocol fields
                protocol_name='TAVNS-COMB — tVNS + tDCS Combination Protocol for TRD',
                clinical_objective='Dual-pathway upregulation: vagal limbic modulation + direct DLPFC facilitation for TRD/severe anxiety',
                primary_targets=['Left cymba concha (tVNS)', 'Left DLPFC F3 (tDCS)'],
                secondary_targets=['LC-NA priming + CEN direct facilitation'],
                device_name='NEMOS/Parasym (tVNS) + Newronika HDCkit (tDCS)',
                session_structure='tVNS 10 min before tDCS; continue tVNS concurrent with tDCS. tVNS: 120 Hz, 250 µs, 30 min. tDCS: F3 anodal, 2 mA, 20 min.',
                frequency='5x/week for 4 weeks, then 3x/week for 2 weeks',
                duration='tVNS 30 min + tDCS 20 min (overlapping)',
                intensity_dose='tVNS: sensory threshold; tDCS: 2 mA',
                montage_roi='tVNS: left cymba concha. tDCS: Anode (+): F3 (L-DLPFC). Cathode (-): Fp2',
                laterality='Left (both modalities)',
                treatment_course='4 weeks intensive + 2 weeks taper',
                monitoring='PHQ-9; GAD-7; heart rate; impedance; skin integrity for both sites',
                expected_response='Enhanced antidepressant/anxiolytic effect via dual-pathway mechanism',
                evidence_status='Low — no published RCT for combination; mechanism-based rationale from component evidence',
                cautions='OFF-LABEL combination. No published combined RCT. Sequential priming rationale. Monitor both modality sites.',
                when_to_escalate='If insufficient response, consider implanted VNS referral',
                when_to_stop_modify='Stop if either modality causes adverse effects',
                clinical_notes_extended='Theoretical synergy: NA upregulation (tVNS) improves signal-to-noise in PFC during tDCS.',
                key_citations=['Hein et al., 2013', 'Brunoni et al., 2017'],
            ),
        ],
        ...
        eeg_reference_table=[
            {
                ...
                "role": 'Concurrent tDCS augmentation target (combination protocol)',
                "protocols_using": ['tVNS-COMB'],
            },
        ],
