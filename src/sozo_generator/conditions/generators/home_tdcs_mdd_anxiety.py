    ...
    return ConditionSchema(
        ...
        protocols=[
            ProtocolEntry(
                ...
                protocol_name='HOME-STD — Home-Based tDCS Standard Depression Protocol',
                clinical_objective='Deliver supervised home-based tDCS for MDD using standard left DLPFC anodal montage',
                primary_targets=['Left DLPFC (F3)'],
                secondary_targets=['Prefrontal-limbic connectivity'],
                device_name='Newronika HDCkit (2-channel) or Sooma tDCS',
                session_structure='Anode (+): F3 (L-DLPFC). Cathode (-): Fp2 (right supraorbital). 2 mA, 30 min. Remote supervision via video.',
                frequency='1 session/day, 5 days/week',
                duration='30 min per session',
                intensity_dose='2 mA',
                montage_roi='Anode (+): F3 (L-DLPFC). Cathode (-): Fp2 (R supraorbital)',
                laterality='Left anodal / Right cathodal',
                treatment_course='20 sessions over 4 weeks; then taper to 3x/week maintenance',
                monitoring='Remote video supervision; PHQ-9 weekly; impedance logging; skin photo check; medication compliance',
                expected_response='PHQ-9 >=50% reduction by Week 4; remission by Week 8',
                evidence_status='Moderate — Alonzo et al. 2019; Charvet et al. 2015; multiple home-tDCS feasibility studies',
                cautions='Remote supervision mandatory for each session. Impedance limits enforced by device. Caregiver involvement recommended.',
                when_to_escalate='If <20% improvement after 10 sessions, reassess diagnosis; consider clinic-based treatment',
                when_to_stop_modify='Stop if skin lesion, non-compliance with supervision, or suicidal ideation increase',
                clinical_notes_extended='Home-based tDCS requires validated remote supervision protocol. Device must have impedance monitoring and session logging.',
                key_citations=['Alonzo et al., 2019', 'Charvet et al., 2015'],
            ),
            ProtocolEntry(
                ...
                # Extended protocol fields
                protocol_name='HOME-ANX — Home-Based tDCS Anxiety Protocol',
                clinical_objective='Deliver supervised home-based tDCS for anxiety using right cathodal / left anodal montage',
                primary_targets=['Right DLPFC (F4) — cathodal', 'Left DLPFC (F3) — anodal'],
                secondary_targets=['Prefrontal-amygdala regulation'],
                device_name='Newronika HDCkit (2-channel) or Sooma tDCS',
                session_structure='Anode (+): F3 (L-DLPFC). Cathode (-): F4 (R-DLPFC). 1.5-2.0 mA, 20-30 min. Remote supervision.',
                frequency='1 session/day, 5 days/week',
                ...
                monitoring='Remote video supervision; GAD-7 weekly; impedance logging; skin photo check',
                expected_response='GAD-7 >=50% reduction by end of treatment block',
                evidence_status='Low-moderate — anxiety home-tDCS extrapolated from clinic-based data',
                cautions='Remote supervision mandatory. Paradoxical anxiety increase possible in first sessions.',
                when_to_escalate='If <20% improvement after 10 sessions, add CES or taVNS adjunct',
                when_to_stop_modify='Stop if skin lesion, non-compliance, or neurological change',
                clinical_notes_extended='Right cathodal / left anodal montage differs from standard depression (which uses Fp2 cathode).',
                key_citations=['Charvet et al., 2015'],
            ),
            ProtocolEntry(
                ...
                # Extended protocol fields
                protocol_name='HOME-SHAM — Sham-Controlled Protocol for Research',
                clinical_objective='Provide sham condition for controlled home-tDCS research studies',
                primary_targets=['Left DLPFC (F3) — sham'],
                secondary_targets=[],
                device_name='Newronika HDCkit (2-channel) or Sooma tDCS (sham mode)',
                session_structure='Same electrode placement as HOME-STD but with 30-second ramp-up/ramp-down sham protocol.',
                frequency='1 session/day, 5 days/week',
                duration='30 min per session (sham)',
                intensity_dose='Sham: 30-second ramp-up then off',
                montage_roi='Same as HOME-STD: Anode (+): F3. Cathode (-): Fp2',
                laterality='Same as active condition',
                treatment_course='Matched to active arm duration',
                monitoring='Same monitoring as active protocol; blinding integrity check',
                expected_response='Placebo response expected (typically 20-30%)',
                evidence_status='Research protocol — sham control for clinical trials',
                cautions='Research use only. Ethics committee approval required. Blinding integrity monitoring.',
                when_to_escalate='N/A — research protocol',
                when_to_stop_modify='Follow trial stopping rules',
                clinical_notes_extended='Sham protocol for blinded controlled trials. 30-second ramp mimics initial tingling sensation.',
                key_citations=['Alonzo et al., 2019'],
            ),
            ProtocolEntry(
                ...
                # Extended protocol fields
                protocol_name='HOME-CES-ADJ — Home CES Adjunct Protocol',
                clinical_objective='Provide home-based CES for sleep, anxiety, and mood as adjunct to tDCS',
                primary_targets=['Bilateral earlobe electrodes'],
                secondary_targets=['Limbic/serotonergic modulation'],
                device_name='Alpha-Stim® AID',
                session_structure='Bilateral earlobe clip electrodes; 0.5 Hz, 100-300 µA; 40-60 min; self-administered after training',
                frequency='Daily',
                duration='40-60 min per session',
                intensity_dose='100-300 µA at 0.5 Hz',
                montage_roi='Bilateral earlobe clip electrodes',
                laterality='Bilateral',
                treatment_course='Daily during tDCS block; then as-needed maintenance',
                monitoring='PHQ-9/GAD-7 weekly; PSQI; skin integrity',
                expected_response='Sleep improvement and anxiolytic effect within 1-2 weeks',
                evidence_status='Moderate — CES FDA-cleared for anxiety, depression, insomnia',
                cautions='Adjunctive only. Self-administered after initial training session. Well-tolerated.',
                when_to_escalate='If insufficient effect, consider adding taVNS',
                when_to_stop_modify='Stop if ear skin irritation',
                clinical_notes_extended='CES can be self-administered at home without supervision after initial training.',
        ...
        eeg_reference_table=[
            {
                ...
                "role": 'Anodal — standard depression target',
                "protocols_using": ['HOME-STD', 'HOME-ANX', 'HOME-SHAM'],
            },
            {
                "position": 'F4',
                "brain_region": 'Right DLPFC',
                "role": 'Cathodal (anxiety) or anodal reference',
                "protocols_using": ['HOME-ANX'],
            },
            {
                "position": 'Fp2',
                "brain_region": 'Right Supraorbital',
                "role": 'Cathodal reference for depression montage',
                "protocols_using": ['HOME-STD', 'HOME-SHAM'],
            },
        ],
