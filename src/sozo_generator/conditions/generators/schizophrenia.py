...
def build_schizophrenia_condition() -> ConditionSchema:
    ...
    return ConditionSchema(
        ...
        eeg_reference_table=[
            {"position": "F3", "brain_region": "Left DLPFC", "role": "Anodal target — CEN upregulation for cognitive deficits and negative symptoms", "protocols_using": ["C-SCZ-BIFRONTAL-TEMPORAL", "C-SCZ-DLPFC-COG"]},
            {"position": "F4", "brain_region": "Right DLPFC", "role": "Bilateral prefrontal extension; right-hemispheric cognitive support", "protocols_using": []},
            {"position": "T3", "brain_region": "Left Temporal / Auditory Cortex", "role": "PRIMARY cathodal target — auditory hallucination generator; Broca's/Wernicke's area inhibition", "protocols_using": ["C-SCZ-BIFRONTAL-TEMPORAL", "TMS-SCZ-1HZ"]},
            {"position": "T4", "brain_region": "Right Temporal / Auditory Cortex", "role": "Bilateral temporal extension for bilateral hallucinations", "protocols_using": []},
            {"position": "Tp7", "brain_region": "Left Temporoparietal Junction", "role": "TPJ — auditory hallucination source region; self-other voice attribution", "protocols_using": ["C-SCZ-BIFRONTAL-TEMPORAL", "TMS-SCZ-1HZ"]},
            {"position": "Tp8", "brain_region": "Right Temporoparietal Junction", "role": "Bilateral TPJ for bilateral hallucination presentations", "protocols_using": []},
        ],
        ...
        protocols=[
            ProtocolEntry(
                ...
                protocol_name="C-SCZ-BIFRONTAL-TEMPORAL — Bifrontal-Temporal tDCS Hallucination & Negative Symptom Protocol",
                clinical_objective="Dual-target: inhibit auditory hallucination generator (L-TPC) + upregulate CEN (L-DLPFC) for negative/cognitive symptoms",
                primary_targets=["F3 (Left DLPFC — anodal)", "T3 or CP5 (Left temporoparietal — cathodal)"],
                secondary_targets=["CEN-DMN balance restoration (indirect)"],
                device_name="Newronika HDCkit (2-channel)",
                session_structure="Anodal F3 (DLPFC); Cathode T3/CP5 (temporoparietal). 2 mA, 20 min. Brunelin: 2 sessions/day × 5 days.",
                frequency="2 sessions/day for 5 days (Brunelin protocol) or 1/day × 10 days",
                duration="20 min per session",
                intensity_dose="2.0 mA",
                montage_roi="Anode (+): F3 (L-DLPFC). Cathode (−): T3 or CP5 (L-temporoparietal)",
                laterality="Left dominant (bifrontal-temporal)",
                treatment_course="10 sessions over 5 days (intensive) or 2 weeks",
                monitoring="PANSS at baseline, Week 2, and endpoint; BPRS at every session; medication stability",
                expected_response="Reduced auditory hallucination frequency/severity; improved negative symptoms; PANSS improvement",
                evidence_status="HIGH — Brunelin et al. 2012 double-blind RCT; Mondino et al. 2015 replication in refractory SCZ",
                cautions="ADJUNCT TO ANTIPSYCHOTICS ONLY. Written psychiatrist clearance mandatory. Any BPRS worsening ≥20% triggers psychiatric review.",
                when_to_escalate="If hallucinations persist, switch to 1 Hz TMS (stronger evidence for medication-resistant hallucinations)",
                when_to_stop_modify="Stop immediately if acute psychotic exacerbation; psychiatric review before resuming",
                clinical_notes_extended="Brunelin protocol: 2 sessions/day with ≥3h interval for 5 days. Intensive schedule is the published protocol.",
                key_citations=["Brunelin et al., 2012", "Mondino et al., 2015", "Kay et al., 1987"],
            ),
            ProtocolEntry(
                ...
                # -- SOZO Protocol Handbook fields --
                protocol_name="TMS-SCZ-1HZ — 1 Hz rTMS for Medication-Resistant Auditory Hallucinations",
                clinical_objective="Inhibit hyperactive left temporal language cortex generating auditory hallucinations",
                primary_targets=["T3/CP5 (Left temporoparietal cortex — 1 Hz inhibitory)"],
                secondary_targets=["Left STG/Wernicke's area (auditory language cortex)"],
                device_name="MagVenture MagPro or MagStim Rapid2",
                session_structure="1 Hz inhibitory rTMS; 90% RMT; 1200 pulses (20 min); T3/CP5 or neuronavigation-guided",
                frequency="5 sessions/week",
                duration="20 min per session",
                intensity_dose="90% RMT; 1200 pulses per session",
                montage_roi="Left temporoparietal cortex (T3/CP5 or neuronavigation-guided)",
                laterality="Left temporal",
                treatment_course="10–20 sessions over 2–4 weeks",
                monitoring="PANSS positive subscale; AHRS (Auditory Hallucination Rating Scale); BPRS at every session",
                expected_response="Reduced hallucination frequency and severity; improved medication response",
                evidence_status="HIGH — Hoffman et al. 2003 landmark RCT; multiple replications; most evidence-based TMS target for hallucinations",
                cautions="ADJUNCT TO ANTIPSYCHOTICS. Medication-resistant hallucinations only. Psychiatric clearance required.",
                when_to_escalate="If no hallucination reduction after 10 sessions, consider clozapine referral",
                when_to_stop_modify="Stop if acute psychotic exacerbation",
                clinical_notes_extended="Most evidence-based neuromodulation approach for medication-resistant auditory hallucinations. Discuss with psychiatrist as alternative to bifrontal-temporal tDCS.",
                key_citations=["Hoffman et al., 2003", "Brunelin et al., 2012"],
            ),
            ProtocolEntry(
                ...
                # -- SOZO Protocol Handbook fields --
                protocol_name="C-SCZ-DLPFC-COG — L-DLPFC Anodal tDCS Cognitive Enhancement Protocol",
                clinical_objective="Upregulate hypofrontal DLPFC circuits to improve working memory, executive function, and negative symptoms",
                primary_targets=["F3 (Left DLPFC — anodal)"],
                secondary_targets=["Right shoulder or Fp2 (cathodal)"],
                device_name="Newronika HDCkit (2-channel)",
                session_structure="Anodal F3; Cathode right shoulder or Fp2. 1.5–2.0 mA, 20 min. Concurrent cognitive remediation training recommended.",
                frequency="5 sessions/week",
                duration="20 min per session",
                intensity_dose="1.5–2.0 mA",
                montage_roi="Anode (+): F3 (L-DLPFC). Cathode (−): right shoulder or Fp2",
                laterality="Left anodal dominant",
                treatment_course="15–20 sessions over 4 weeks",
                monitoring="PANSS negative subscale; SANS; cognitive battery (working memory, processing speed); BPRS at every session",
                expected_response="Improved working memory, processing speed, executive function; reduced negative symptoms",
                evidence_status="Medium — Cordes et al. 2015 working memory improvement; best combined with cognitive remediation",
                cautions="ADJUNCT TO ANTIPSYCHOTICS. Cognitive remediation during stimulation enhances outcomes.",
                when_to_escalate="If no cognitive improvement after 10 sessions, reassess phenotype and consider medication optimization",
                when_to_stop_modify="Stop if BPRS positive symptom worsening ≥20%",
                clinical_notes_extended="Cognitive impairment is the strongest predictor of functional outcome in schizophrenia. Combine with cognitive remediation training during stimulation.",
                key_citations=["Cordes et al., 2015", "Brunelin et al., 2012"],
            ),
        ],
        ...
        platoscience_variants=[
            PlatoScienceVariant(
                variant_id="C-SCZ-BIFRONTAL-TEMPORAL-PS", protocol_id="C-SCZ-BIFRONTAL-TEMPORAL",
                label="PlatoScience Bifrontal-Temporal tDCS — Hallucination & Negative Symptom Protocol",
                parameters={"program": "Focus", "electrode_config": "F3 (left DLPFC)", "intensity": "2.0 mA", "duration": "20 min", "ramp": "30 sec"},
                notes="Maps to C-SCZ-BIFRONTAL-TEMPORAL protocol. PlatoScience Focus program.",
            ),
            PlatoScienceVariant(
                variant_id="C-SCZ-DLPFC-COG-PS", protocol_id="C-SCZ-DLPFC-COG",
                label="PlatoScience L-DLPFC Anodal tDCS — Cognitive Enhancement",
                parameters={"program": "Think", "electrode_config": "F3 (left DLPFC)", "intensity": "1.5-2.0 mA", "duration": "20 min", "ramp": "30 sec"},
                notes="Maps to C-SCZ-DLPFC-COG protocol. PlatoScience Think program.",
            ),
        ],
