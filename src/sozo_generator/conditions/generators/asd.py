    ...
    return ConditionSchema(
        ...
        protocols=[
            ProtocolEntry(
                ...
                protocol_name='TDCS-ASD-EXEC — Executive Function Protocol (INVESTIGATIONAL)',
                clinical_objective='Enhance executive function (working memory, flexibility, planning) in ASD via left DLPFC anodal stimulation',
                primary_targets=['Left DLPFC (F3)'],
                secondary_targets=['Frontostriatal executive circuits'],
                device_name='Newronika HDCkit (2-channel)',
                session_structure='Anode (+): F3 (L-DLPFC). Cathode (-): Fp2 (right supraorbital) or right shoulder. 1.5-2.0 mA (start 1.5), 20 min.',
                frequency='1 session/day, 5 days/week',
                duration='20 min per session',
                intensity_dose='1.5-2.0 mA (start 1.5 mA for sensory sensitivity)',
                ...
                monitoring='Impedance check; BRIEF-2 pre/post block; SRS-2; behavioral monitoring for distress; skin integrity',
                expected_response='Improved working memory and cognitive flexibility by Week 3-4',
                evidence_status="Low — D'Urso et al. 2015 pilot (N<20); Schneider & Hopp 2011; all pilot-level",
                cautions='INVESTIGATIONAL/OFF-LABEL. Elevated epilepsy risk in ASD (~20-30%) — screen before treatment. Sensory sensitivity may cause electrode distress. Start 1.5 mA.',
                when_to_escalate='If <20% improvement after 10 sessions, consider TPJ social cognition protocol or ADHD-focused protocol if co-occurring',
                when_to_stop_modify='Stop if seizure activity, severe distress, skin lesion, or increased stereotypies',
                clinical_notes_extended='INVESTIGATIONAL. Informed consent must specify off-label status. Allow extra electrode preparation time for sensory sensitivity. Concurrent cognitive training recommended.',
                key_citations=["D'Urso et al., 2015", 'Schneider & Hopp, 2011'],
            ),
            ProtocolEntry(
                ...
                # Extended protocol fields
                protocol_name='TDCS-ASD-SOC — Social Cognition TPJ Protocol (INVESTIGATIONAL)',
                clinical_objective='Enhance social cognition and mentalizing via right TPJ anodal stimulation',
                primary_targets=['Right TPJ (P4 approximation)'],
                secondary_targets=['Social brain network (STS, mPFC)'],
                device_name='Newronika HDCkit (2-channel)',
                session_structure='Anode (+): P4 (right TPJ approximation). Cathode (-): F3 or Fp1. 1.5 mA, 20 min.',
                frequency='1 session/day, 5 days/week',
                duration='20 min per session',
                intensity_dose='1.5 mA',
                montage_roi='Anode (+): P4 (R-TPJ). Cathode (-): F3 (L-DLPFC) or Fp1',
                laterality='Right parietal',
                treatment_course='10-15 sessions over 2-3 weeks',
                monitoring='Impedance check; SRS-2 pre/post block; theory of mind tasks; behavioral monitoring; skin integrity',
                expected_response='Improved social perspective-taking and mentalizing scores',
                evidence_status='Very low — rationale from neuroimaging models; very limited published tDCS data for TPJ in ASD',
                cautions='PURELY INVESTIGATIONAL. Very limited evidence. Requires Doctor authorization and explicit informed consent. Screen for epilepsy.',
                when_to_escalate='If no improvement, consider DLPFC executive function protocol instead',
                when_to_stop_modify='Stop if seizure activity, severe distress, or skin lesion',
                clinical_notes_extended='PURELY INVESTIGATIONAL. Concurrent social skills or perspective-taking training recommended during stimulation.',
                key_citations=['Enticott et al., 2014'],
            ),
            ProtocolEntry(
                ...
                # Extended protocol fields
                protocol_name='CES-ASD — Anxiety & Sleep Adjunct for ASD',
                clinical_objective='Address highly prevalent anxiety (40-80%) and sleep disturbance (50-80%) in adult ASD',
                ...
                monitoring='GAD-7 weekly; sleep diary; behavioral monitoring; skin integrity',
                expected_response='Anxiety reduction and sleep improvement within 1-2 weeks',
                evidence_status='Moderate — CES FDA-cleared for anxiety and insomnia; limited ASD-specific data but targets prevalent comorbidities',
                cautions='Adjunctive only. Best-evidenced component for ASD comorbid symptoms. Sensory sensitivity with earlobe clips — allow familiarization.',
                when_to_escalate='If anxiety persists, add taVNS for autonomic regulation',
                when_to_stop_modify='Stop if ear skin irritation or distress during use',
                clinical_notes_extended='Particularly relevant for autistic burnout and anxiety. Non-pharmacological option for the highly prevalent sleep/anxiety comorbidities.',
        ...
        eeg_reference_table=[
            {
                ...
                "role": 'Anodal — executive function enhancement',
                "protocols_using": ['C-ASD-EXEC'],
            },
            {
                "position": 'F4',
                "brain_region": 'Right DLPFC',
                "role": 'Reference for TPJ montage',
                "protocols_using": ['C-ASD-SOC'],
            },
            {
                "position": 'P4',
                "brain_region": 'Right TPJ (approximation)',
                "role": 'Anodal — social cognition / mentalizing',
                "protocols_using": ['C-ASD-SOC'],
            },
            {
                "position": 'T3',
                "brain_region": 'Left Temporal / STS proxy',
                "role": 'Social brain network node',
                "protocols_using": [],
            },
            {
                "position": 'T4',
                "brain_region": 'Right Temporal / STS proxy',
                "role": 'Social brain network node',
                "protocols_using": [],
            },
        ],
