...
def build_chronic_pain_condition() -> ConditionSchema:
    ...
    return ConditionSchema(
        ...
        eeg_reference_table=[
            {"position": "C3", "brain_region": "Left Primary Motor Cortex (M1)", "role": "PRIMARY anodal target — descending pain modulation via corticospinal-PAG circuit", "protocols_using": ["C-PAIN-M1"]},
            {"position": "C4", "brain_region": "Right Primary Motor Cortex (M1)", "role": "Bilateral M1 anodal target; contralateral pain modulation", "protocols_using": ["C-PAIN-M1"]},
            {"position": "F3", "brain_region": "Left DLPFC", "role": "Cognitive-emotional pain control; comorbid depression target; catastrophizing reduction", "protocols_using": ["C-PAIN-DLPFC"]},
            {"position": "Fz", "brain_region": "Midline Frontal / ACC", "role": "Affective-motivational pain component; ACC hyperactivation in chronic pain", "protocols_using": []},
            {"position": "T3", "brain_region": "Left Temporal (Insular/Thalamic Proxy)", "role": "Insular cortex approximation — interoceptive pain processing; SN hyperactivation", "protocols_using": []},
            {"position": "T4", "brain_region": "Right Temporal (Insular/Thalamic Proxy)", "role": "Bilateral insular access for central sensitization modulation", "protocols_using": []},
        ],
        ...
        protocols=[
            ProtocolEntry(
                ...
                protocol_name="C-PAIN-M1 — Motor Cortex Analgesic Protocol",
                clinical_objective="Activate descending pain inhibitory pathways via M1-PAG corticospinal circuit for analgesic effect",
                primary_targets=["C3 + C4 (Bilateral M1 — anodal)", "C3 alone (Left M1 — Fregni montage)"],
                secondary_targets=["Fp1 + Fp2 (bilateral supraorbital — cathodal)", "Contralateral shoulder (cathodal)"],
                device_name="Newronika HDCkit (2-channel)",
                session_structure="Ch1: Anodal C3 (+/- C4); Cathode: Fp1+Fp2 or contralateral shoulder. 2 mA, 20–30 min.",
                frequency="5 sessions/week",
                duration="20–30 min per session",
                intensity_dose="2.0 mA",
                montage_roi="Anode (+): C3 + C4 (bilateral M1) or C3 alone. Cathode (−): Fp1+Fp2 or shoulder",
                laterality="Bilateral or left dominant (Fregni montage)",
                treatment_course="10–20 sessions over 4–5 weeks",
                monitoring="NRS pain at every session; FIQR at Week 4 and Week 8; skin integrity; medication stability",
                expected_response="≥30% NRS pain reduction; improved function; reduced central sensitization",
                evidence_status="Level B (IFCN) for fibromyalgia — Fregni et al. 2006 RCT; Marlow et al. 2013 meta-analysis",
                cautions="Transient pain increase possible in first 1–2 sessions (sensitized cortex); reduce to 1.5 mA if persistent",
                when_to_escalate="If <30% pain reduction at Week 4, add DLPFC protocol or CES for sleep",
                when_to_stop_modify="Standard tDCS stop criteria; reduce intensity if pain worsening persists beyond 3 sessions",
                clinical_notes_extended="M1 anodal mechanism: corticospinal activation of brainstem pain modulation circuits (PAG, RVM). Document analgesic medication stability.",
                key_citations=["Fregni et al., 2006", "Marlow et al., 2013", "Lefaucheur et al., 2017"],
            ),
            ProtocolEntry(
                ...
                # -- SOZO Protocol Handbook fields --
                protocol_name="C-PAIN-DLPFC — Left DLPFC Cognitive Pain & Depression Protocol",
                clinical_objective="Reduce catastrophizing and improve cognitive pain coping; address comorbid depression",
                primary_targets=["F3 (Left DLPFC — anodal)"],
                secondary_targets=["Fp2 (right supraorbital — cathodal)"],
                device_name="Newronika HDCkit (2-channel)",
                session_structure="Anodal F3; Cathode Fp2. 2 mA, 30 min. Monitor PHQ-9 at every session.",
                frequency="5 sessions/week",
                duration="30 min per session",
                intensity_dose="2.0 mA",
                montage_roi="Anode (+): F3 (L-DLPFC). Cathode (−): Fp2 (right supraorbital)",
                laterality="Left anodal dominant",
                treatment_course="10–15 sessions over 3 weeks",
                monitoring="PHQ-9 at every session; NRS pain; catastrophizing scale (PCS)",
                expected_response="Reduced catastrophizing, improved pain coping, PHQ-9 improvement",
                evidence_status="Medium — evidence from MDD tDCS literature extrapolated to pain-depression comorbidity",
                cautions="MDD-adapted montage; monitor mood carefully in pain-depression subtype",
                when_to_escalate="If depression not improving, consider dual M1+DLPFC sequential protocol",
                when_to_stop_modify="Standard tDCS stop criteria",
                clinical_notes_extended="For pain-depression comorbidity phenotype. Catastrophizing is a major pain amplifier — CEN upregulation reduces catastrophizing.",
                key_citations=["Fregni et al., 2006", "Knotkova et al., 2017"],
            ),
            ProtocolEntry(
                ...
                # -- SOZO Protocol Handbook fields --
                protocol_name="CES-PAIN — Alpha-Stim CES Sleep, Anxiety & Mood Adjunct for Chronic Pain",
                clinical_objective="Address sleep disturbance, anxiety, and mood symptoms that amplify pain experience",
                ...
                intensity_dose="100–300 µA",
                montage_roi="Bilateral earlobe clip electrodes",
                laterality="Bilateral",
                treatment_course="Daily for duration of tDCS block (4–5 weeks)",
                monitoring="PSQI at Week 4; NRS pain; PHQ-9; skin integrity at earlobe sites",
                expected_response="Improved sleep quality, reduced anxiety, improved mood — indirect pain reduction via sleep restoration",
                evidence_status="Medium — FDA-cleared for anxiety, depression, insomnia; directly addresses FMS comorbidities",
                cautions="Non-pharmacological — reduces medication burden; adjunct to tDCS pain protocols",
                ...
                clinical_notes_extended="Sleep restoration markedly improves pain treatment response. Prescribe for home use alongside tDCS clinic sessions.",
            ...
            ProtocolEntry(
                ...
                # -- SOZO Protocol Handbook fields --
                protocol_name="TAVNS-PAIN — taVNS Autonomic Pain Modulation Adjunct",
                clinical_objective="Modulate autonomic pain regulation via NTS-PAG pathways; reduce central sensitization",
                primary_targets=["Auricular branch of vagus nerve → NTS → PAG descending inhibition"],
                secondary_targets=["Anti-inflammatory vagal pathway", "Limbic modulation (indirect)"],
                device_name="NEMOS (tVNS Technologies) or Parasym",
                session_structure="Electrode placement on left cymba conchae; 25 Hz, 250 µs pulse width; below pain threshold; 30 min",
                frequency="Daily adjunct during tDCS block",
                duration="30 min per session",
                intensity_dose="Below pain threshold (titrate to sensory threshold)",
                montage_roi="Left cymba conchae (auricular vagus branch)",
                laterality="Left auricular",
                treatment_course="Daily for duration of tDCS block (4–5 weeks)",
                monitoring="Pain threshold check; NRS pain; heart rate monitoring",
                expected_response="Reduced central sensitization via enhanced descending inhibition; autonomic balance improvement",
                evidence_status="Low — investigational for chronic pain; rationale from pain modulation literature",
                cautions="No dedicated fibromyalgia RCT; adjunct only; informed consent required",
                when_to_escalate="If no improvement, focus on primary tDCS protocol optimization",
                when_to_stop_modify="Stop if vagal side effects (bradycardia, cough)",
                clinical_notes_extended="Reduces autonomic hyperreactivity contributing to central sensitization. Investigational — limited published data for chronic pain.",
                key_citations=["Jensen et al., 2014"],
            ),
        ],
        ...
        platoscience_variants=[
            PlatoScienceVariant(
                variant_id="C-PAIN-M1-PS", protocol_id="C-PAIN-M1",
                label="PlatoScience M1 Analgesic — Fibromyalgia / Chronic Pain",
                parameters={"program": "Focus", "electrode_config": "C3 + C4 (bilateral M1) or C3 alone (left M1)", "intensity": "2.0 mA", "duration": "20-30 min", "ramp": "30 sec"},
                notes="Maps to C-PAIN-M1 protocol. PlatoScience Focus program.",
            ),
            PlatoScienceVariant(
                variant_id="C-PAIN-DLPFC-PS", protocol_id="C-PAIN-DLPFC",
                label="PlatoScience Left DLPFC — Cognitive Pain & Depression",
                parameters={"program": "Think", "electrode_config": "F3 (left DLPFC)", "intensity": "2.0 mA", "duration": "30 min", "ramp": "30 sec"},
                notes="Maps to C-PAIN-DLPFC protocol. PlatoScience Think program.",
            ),
        ],
