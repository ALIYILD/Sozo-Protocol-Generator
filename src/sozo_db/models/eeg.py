...
class EEGRecord(UUIDMixin, Base):
    ...
    band_powers: Mapped[dict | None] = mapped_column(JSON_VARIANT, nullable=True)
    asymmetry_indices: Mapped[dict | None] = mapped_column(JSON_VARIANT, nullable=True)
    peak_alpha_frequency: Mapped[float | None] = mapped_column(Float, nullable=True)
    z_scores: Mapped[dict | None] = mapped_column(JSON_VARIANT, nullable=True)
    connectivity: Mapped[dict | None] = mapped_column(JSON_VARIANT, nullable=True)
    source_localization: Mapped[dict | None] = mapped_column(JSON_VARIANT, nullable=True)
    source_file_path: Mapped[str | None] = mapped_column(String(1024), nullable=True)
    processing_params: Mapped[dict | None] = mapped_column(JSON_VARIANT, nullable=True)
